...
cCamera::cCamera(eProj proj, const tVector &pos, const tVector &focus,
                 const tVector &up, double w, double h, double near_z,
                 double far_z)
{
    mPosition = pos;
...
const tVector &cCamera::GetPosition() const { return mPosition; }

tVector cCamera::GetFocus() const { return mFocusDelta + mPosition; }

const tVector &cCamera::GetUp() const { return mUp; }
...
double cCamera::GetWidth() const { return mWidth; }
double cCamera::GetHeight() const { return mWidth / mAspectRatio; }

double cCamera::GetAspectRatio() const { return mAspectRatio; }

double cCamera::GetNearZ() const { return mNearZ; }

double cCamera::GetFarZ() const { return mFarZ; }
...
void cCamera::SetUp(const tVector &up) { mUp = up; }
...
void cCamera::TranslatePos(const tVector &pos) { mPosition = pos; }
...
tVector cCamera::ScreenToWorldPos(const tVector &screen_pos) const
{
    ...
    double focal_len = CalcFocalLen();
    tVector view_pos = tVector(0.5 * screen_pos[0] * w, 0.5 * screen_pos[1] * h,
                               -focal_len, 1);
...
void cCamera::SetProj(eProj proj) { mProj = proj; }

cCamera::eProj cCamera::GetProj() const { return mProj; }
    ...
    if (mMouseDown)
        {
            ...
            if (mouse_mod & GLUT_ACTIVE_ALT)
            ...
            else
                {
                    // rotate
                    tMatrix rot_mat =
                        cMathUtil::RotateMat(tVector(0, 1, 0, 0), -M_PI * dx) *
...
double cCamera::CalcFocalLen() const { return mFocusDelta.norm(); }
