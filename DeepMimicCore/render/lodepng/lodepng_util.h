...
struct ZlibBlockInfo
{
    ...
    std::vector<int>
        clcl; // 19 code length code lengths (compressed tree's tree)
    std::vector<int> treecodes; // N tree codes, with values 0-18. Values 17 or
                                // 18 are followed by the repetition value.
    ...
    std::vector<int>
        lz77_lcode; // LZ77 codes. 0-255: literals. 256: end symbol. 257-285:
                    // length code of length/dist pairs
    // the next vectors have the same size as lz77_lcode, but an element only
    // has meaningful value if lz77_lcode contains a length code.
...
// Extracts all info needed from a PNG file to reconstruct the zlib compression
// exactly.
