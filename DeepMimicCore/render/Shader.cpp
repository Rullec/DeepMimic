...
std::stack<cShader::tShaderEntry> cShader::gShaderStack =
    std::stack<tShaderEntry>();

const std::string gUniformNames[cShader::eUniformMax] = {
    "uModelViewMatrix", "uProjectionMatrix", "uColor"};

cShader::tShaderEntry::tShaderEntry() { mShader = nullptr; }
...
cShader::cShader(void) { mProg = gInvalidIdx; }

cShader::~cShader(void) { Clear(); }
...
bool cShader::BuildShader(const std::string &vs_filename,
...
GLuint cShader::LoadShader(const std::string &filename, GLenum shader_type)
{
    ...
    if (!compiled)
        {
            std::cout << "shader compilation failed: " << filename.c_str()
                      << std::endl;
...
void cShader::GetAttributeHandle(GLuint &attribute_handle,
                                 const std::string &attribute_name)
{
    attribute_handle = glGetAttribLocation(mProg, attribute_name.c_str());
    if (attribute_handle == gInvalidIdx)
        {
            std::cout << "can't find shader attribute "
                      << attribute_name.c_str() << std::endl;
        }
}

void cShader::GetUniformHandle(GLuint &uniform_handle,
                               ...
                               bool check_valid /*= false*/)
{
    ...
    if (check_valid && uniform_handle == gInvalidIdx)
        {
            std::cout << "can't find shader uniform parameter "
                      << uniform_name.c_str() << std::endl;
        }
}

void cShader::Bind() const { PushShaderStack(); }

void cShader::Unbind() const { PopShaderStack(); }
...
{
    float f_data[] = {
        static_cast<float>(data(0, 0)), static_cast<float>(data(1, 0)),
        static_cast<float>(data(2, 0)), static_cast<float>(data(3, 0)),
        static_cast<float>(data(0, 1)), static_cast<float>(data(1, 1)),
        static_cast<float>(data(2, 1)), static_cast<float>(data(3, 1)),
        static_cast<float>(data(0, 2)), static_cast<float>(data(1, 2)),
        static_cast<float>(data(2, 2)), static_cast<float>(data(3, 2)),
        static_cast<float>(data(0, 3)), static_cast<float>(data(1, 3)),
        static_cast<float>(data(2, 3)), static_cast<float>(data(3, 3))};
    ...
    if (bound)
    ...
    else
        {
            throw std::runtime_error(
                "Trying to pop unbound shader from stack.\n");
        }
}