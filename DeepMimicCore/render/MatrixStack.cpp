...
const tMatrix &cMatrixStack::GetMatrix() const { return mCurrMat; }
...
size_t cMatrixStack::GetCapacity() const { return mMatStack.size(); }

size_t cMatrixStack::GetSize() const { return mStackHead; }

void cMatrixStack::SetIdentity() { mCurrMat.setIdentity(); }

void cMatrixStack::SetMatrix(const tMatrix &mat) { mCurrMat = mat; }

void cMatrixStack::MultMatrix(const tMatrix &mat) { mCurrMat = mCurrMat * mat; }
...
void cMatrixStack::Translate(const tVector &trans)
{
     ...
     */
    mCurrMat.block(0, 3, 3, 1) +=
        mCurrMat.block(0, 0, 3, 3) * trans.segment(0, 3);
}
...
void cMatrixStack::Rotate(double theta, const tVector &axis)
{
    ...
    mCurrMat.block(0, 0, 3, 3) =
        mCurrMat.block(0, 0, 3, 3) * rot_mat.block(0, 0, 3, 3);
}