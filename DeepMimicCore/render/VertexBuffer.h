 ...
 * can handle many different vertex attribute layouts.
 */
class cVertexBuffer
{
public:
    ...
    cVertexBuffer()
        : mRenderID(0), mRenderState(NULL), mLocalData(NULL), mAttrInfo(NULL),
          mNumAttr(0), mSize(0)
    {
    }
    cVertexBuffer(GLuint bufferID, cRenderState &r_state)
        : mRenderID(bufferID), mRenderState(&r_state), mLocalData(NULL),
          mAttrInfo(NULL), mNumAttr(0), mSize(0)
    {
    }
    cVertexBuffer(const cVertexBuffer &old)
        : mRenderID(old.mRenderID), mRenderState(old.mRenderState),
          mNumAttr(old.mNumAttr), mSize(old.mSize)
    {
    ...
    // Since the buffer object has no notion of VAO it is just setting its
    // parameters on whatever VAO is currently bound
    ...
    float *mLocalData; // our local copy of the mesh data that we can modify and
                       // copy to the gpu
