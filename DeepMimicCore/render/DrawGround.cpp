...
{
    const cGroundPlane *ground_plane =
        reinterpret_cast<const cGroundPlane *>(ground);
    ...
    tVector a = tVector(center[0] - 0.5 * size[0], ground_origin[1],
                        center[2] - 0.5 * size[2], 0);
    tVector b = tVector(center[0] - 0.5 * size[0], ground_origin[1],
                        center[2] + 0.5 * size[2], 0);
    tVector c = tVector(center[0] + 0.5 * size[0], ground_origin[1],
                        center[2] + 0.5 * size[2], 0);
    tVector d = tVector(center[0] + 0.5 * size[0], ground_origin[1],
                        center[2] - 0.5 * size[2], 0);
    ...
    float vert_data[vert_len] = {
        ...
        static_cast<float>(c[2]), static_cast<float>(c[0]),
        static_cast<float>(c[1]), static_cast<float>(c[2]),
        static_cast<float>(d[0]), static_cast<float>(d[1]),
        static_cast<float>(d[2]), static_cast<float>(a[0]),
        static_cast<float>(a[1]), static_cast<float>(a[2])};

    float norm_data[norm_len] = {0.f, 1.f, 0.f, 0.f, 1.f, 0.f, 0.f, 1.f, 0.f,
    ...
    cMeshUtil::BuildDrawMesh(vert_data, vert_len, norm_data, norm_len,
                             coord_data, coord_len, idx_data, idx_len,
                             out_mesh);
}