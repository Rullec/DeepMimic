...
void cDrawSimCharacter::Draw(const cSimCharacterBase &character,
...
void cDrawSimCharacter::DrawCoM(const cSimCharacterBase &character,
                                double marker_size, double vel_scale,
                                const tVector &col, const tVector &offset)
{
    ...
    cDrawUtil::DrawArrow2D(com + offset, com + offset + com_vel * vel_scale,
                           arrow_size);
}

void cDrawSimCharacter::DrawTorque(const cSimCharacterBase &character,
...
void cDrawSimCharacter::DrawBodyVel(const cSimCharacterBase &character,
...
void cDrawSimCharacter::DrawInfoValLog(const cCircularBuffer<double> &val_log,
                                       const cCamera &cam)
{
            ...
            for (int i = 1; i < num_val; ++i)
                {
                    ...
                    a[1] = h * cMathUtil::Clamp((prev_val - min_val) /
                                                    (max_val - min_val),
                                                0.0, 1.0);
                    b[1] = h * cMathUtil::Clamp((curr_val - min_val) /
                                                    (max_val - min_val),
                                                0.0, 1.0);
...
void cDrawSimCharacter::DrawSimBody(const cSimCharacterBase &character,
