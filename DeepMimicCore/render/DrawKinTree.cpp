...
void cDrawKinTree::Draw(const Eigen::MatrixXd &joint_desc,
                        const Eigen::VectorXd &pose, double link_width,
...
void cDrawKinTree::DrawTree(const Eigen::MatrixXd &joint_desc,
                            const Eigen::VectorXd &pose, int joint_id,
                            double link_width, const tVector &fill_col,
    ...
    if (joint_id != cKinTree::gInvalidJointID)
        {
            ...
            if (has_parent)
                {
                    tVector attach_pt =
                        cKinTree::GetAttachPt(joint_desc, joint_id);
                    ...
                    cDrawUtil::SetColor(tVector(fill_col[0], fill_col[1],
                                                fill_col[2], fill_col[3]));
                    cDrawUtil::DrawBox(tVector::Zero(), size,
                                       cDrawUtil::eDrawSolid);

                    if (line_col[3] > 0)
                        {
                            cDrawUtil::SetColor(
                                tVector(line_col[0], line_col[1], line_col[2],
                                        line_col[3]));
                            cDrawUtil::DrawBox(tVector::Zero(), size,
            ...
            // draw node
            cDrawUtil::SetColor(tVector(fill_col[0] * 0.25, fill_col[1] * 0.25,
                                        fill_col[2] * 0.25, fill_col[3]));
            ...
            for (int i = 0; i < children.size(); ++i)
                {
                    ...
                    DrawTree(joint_desc, pose, child_id, link_width, fill_col,
                             line_col);
                }
