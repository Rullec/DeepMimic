...
std::stack<cTextureDesc::tTexEntry> cTextureDesc::gTexStack =
    std::stack<cTextureDesc::tTexEntry>();
...
cTextureDesc::cTextureDesc(int width, int height, int channels, GLenum format,
    ...
    : mWidth(width), mHeight(height), mDepth(1), mChannels(channels),
      mFormat(format), mType(type), mHasMips(mipmaps)
{
    cTextureUtil::CreateFrameBuffer(mObject, mTexture, mDepthStencil, mWidth,
                                    mHeight, mDepth, mChannels, mFormat, mType,
                                    mHasMips);
}

cTextureDesc::cTextureDesc(int width, int height, int depth, int channels,
                           GLenum format, GLenum type, bool mipmaps)
    : mWidth(width), mHeight(height), mDepth(depth), mChannels(channels),
      mFormat(format), mType(type), mHasMips(mipmaps)
{
    cTextureUtil::CreateFrameBuffer(mObject, mTexture, mDepthStencil, mWidth,
                                    mHeight, mDepth, mChannels, mFormat, mType,
...
cTextureDesc::cTextureDesc(GLuint obj, GLuint tex, GLuint ds, int width,
                           int height, int depth, int channels, GLenum format)
    : mWidth(width), mHeight(height), mDepth(depth), mChannels(channels),
      mFormat(format), mObject(obj), mTexture(tex), mDepthStencil(ds)
{
}

cTextureDesc::cTextureDesc(const std::string &filename, bool gen_mips)
    : cTextureDesc()
{
    ...
    if (error == 0)
        {
            ...
            mHasMips = true;
            int num_mipmaps =
                (gen_mips) ? static_cast<int>(log2(std::min(mWidth, mHeight)))
                           : 1;

            glGenTextures(1, &mTexture);
            glBindTexture(GL_TEXTURE_2D, mTexture);
            glTexStorage2D(GL_TEXTURE_2D, num_mipmaps, mChannels, mWidth,
                           mHeight);
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, mWidth, mHeight, mFormat,
                            mType, image.data());
...
void cTextureDesc::Reshape(int w, int h)
{
    ...
    mHeight = std::max(1, h);
    if (mObject !=
        0) // 0 indicates the device's frame buffer, so no need to resize it
        {
            cTextureUtil::DeleteFrameBuffer(mObject, mTexture, mDepthStencil);
            cTextureUtil::CreateFrameBuffer(mObject, mTexture, mDepthStencil,
                                            mWidth, mHeight, mDepth, mChannels,
                                            mFormat, mType, mHasMips);
        }
}
...
{
    glBindTexture(GL_TEXTURE_2D, mTexture);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, mWidth, mHeight, mFormat, mType,
                    data.data());
    glBindTexture(GL_TEXTURE_2D, 0);
}
            ...
            if (!gTexStack.empty())
                {
                    ...
                    glViewport(prev_entry.mViewportParams[0],
                               prev_entry.mViewportParams[1],
                               prev_entry.mViewportParams[2],
                               prev_entry.mViewportParams[3]);
                }
        }
    else
        {
            throw std::runtime_error(
                "Trying to pop unbound texture from stack.\n");
        }
}
