...
void cTextureUtil::CreateFrameBuffer(GLuint &buffer_obj, GLuint &texture,
                                     GLuint &depth_stencil, int width,
                                     ...
                                     GLenum format, GLenum type, bool mipmaps)
{
    ...
    if (is_3d_tex)
        {
            glTexImage3D(GL_TEXTURE_3D, 0, channels, width, height, depth, 0,
                         format, type, 0);
        }
    else
        {
    ...
    if (mipmaps)
        {
            glTexParameteri(tex_type, GL_TEXTURE_MIN_FILTER,
    ...
    if (is_3d_tex)
        {
            ...
            glFramebufferTexture(GL_RENDERBUFFER, GL_COLOR_ATTACHMENT0, texture,
                                 0);
        }
    else
        {
...
void cTextureUtil::DeleteFrameBuffer(GLuint &buffer_obj, GLuint &texture,
...
tVector cTextureUtil::ReadTexel(int x, int y, int w, int h,
