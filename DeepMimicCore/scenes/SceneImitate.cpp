...
//    return;
    ...
    param.joint_names = GetCharacter()->GetJointNames();
    param.link_names  = GetCharacter()->GetBodyNames();
    param.body_shape_param = body_param;

    RunRetargeting(param);
    if (!mRetOptImpl->IsStdJointMatSet()) {
        Eigen::MatrixXd std_joint_mat = GetCharacter()->GetStdJointMat();
        mRetOptImpl->SetStdJointMat(std_joint_mat);
    }
}

void cSceneImitate::DumpMotionPool(const char *dir) {
    if (!mRetOptImpl) return;

    auto* shape_pool = mRetOptImpl->GetShapeMotionPool();
    auto pool_size   = shape_pool->GetLength();
    Eigen::MatrixXd joint_mat = GetCharacter()->GetStdJointMat();
    Eigen::MatrixXd motion_mat = GetKinChar()->GetStdMotionFrames();
    Json::Value data_root;
    data_root["nodes"] = Json::Value();
    data_root["dir"] = "/home/ljf/playground/DeepMimic/" ;

    std::string origin_motion_dir(dir);
    origin_motion_dir.append("/origin_motion.txt");
    std::string origin_skeleton_dir(dir);
    origin_skeleton_dir.append("/origin_skeleton.txt");

    data_root["origin_motion"] = origin_motion_dir.data();
    data_root["origin_skeleton"] = origin_skeleton_dir.data();

    mRetOptImpl->SaveJointMat(origin_skeleton_dir.data(), joint_mat);
    mRetOptImpl->SaveMotionMat(origin_motion_dir.data(), motion_mat);

    for(auto i = 0; i < pool_size; ++i) {

        auto* p = shape_pool->GetShapeMotionNode(i);
        std::string motion_path(dir);
        motion_path.append("/motion_");
        motion_path.append(std::to_string(i));
        motion_path.append(".txt");
        mRetOptImpl->SaveMotionMat(motion_path.data(), p->motion_mat);

        std::string skeleton_path(dir);
        skeleton_path.append("/joint_");
        skeleton_path.append(std::to_string(i));
        skeleton_path.append(".txt");
        GetCharacter()->ChangeBodyShape(p->body_shape_param, joint_mat);
        mRetOptImpl->SaveJointMat(skeleton_path.data(), joint_mat);

        std::string param_path(dir);
        param_path.append("/param");
        param_path.append(std::to_string(i));
        param_path.append(".txt");
        mRetOptImpl->SaveShapeParam(param_path.data(), p->body_shape_param);

        Json::Value node;
        node["joint_name"] = skeleton_path;
        node["motion_name"]= motion_path;
        node["param_name"] = param_path;
        data_root["nodes"].append(node);
    }

    std::string json_path(dir);
    json_path.append("/list.json");
    std::ofstream fout(json_path.data());

    Json::StreamWriterBuilder builder;
    auto* writer = builder.newStreamWriter();
    writer->write(data_root, &fout);
    fout.close();
}
