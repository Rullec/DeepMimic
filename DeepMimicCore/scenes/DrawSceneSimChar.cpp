...
void cDrawSceneSimChar::MouseClick(int button, int state, double x, double y)
{
    ...
    if (button == GLUT_LEFT_BUTTON)
    {
        if (state == GLUT_DOWN)
        {
            ...
            cFeaWorld::tRayTestResult raytest_result;
...
void cDrawSceneSimChar::Keyboard(unsigned char key, double device_x,
                                 double device_y)
{
...
double cDrawSceneSimChar::GetTime() const { return mScene->GetTime(); }

bool cDrawSceneSimChar::IsEpisodeEnd() const { return mScene->IsEpisodeEnd(); }
...
std::string cDrawSceneSimChar::GetName() const { return mScene->GetName(); }

void cDrawSceneSimChar::BuildScene(
...
tVector cDrawSceneSimChar::GetCamStillPos() const
{
    ...
    for (int i = 0; i < num_samples; ++i)
    {
        ...
        pos[0] = static_cast<double>(i) / (num_samples - 1) * (max_x - min_x) +
                 min_x;
...
tVector cDrawSceneSimChar::GetDefaultCamFocus() const { return gCamFocus0; }
...
void cDrawSceneSimChar::AddCharTrace(
    const std::shared_ptr<cSimCharacterBase> &character, const tVectorArr &cols)
{
...
void cDrawSceneSimChar::SpawnProjectile() { mScene->SpawnProjectile(); }

void cDrawSceneSimChar::SpawnBigProjectile() { mScene->SpawnBigProjectile(); }
...
void cDrawSceneSimChar::RayTest(const tVector &start, const tVector &end,
                                cFeaWorld::tRayTestResult &out_result)
{
...
void cDrawSceneSimChar::HandleRayTest(const cFeaWorld::tRayTestResult &result)
{
    if (result.mObj != nullptr)
    {
        ...
        if (obj_type == cSimObj::eTypeDynamic)
        {
            ...
            {
                mSelectObjLocalPos =
                    mSelectedObj->WorldToLocalPos(result.mHitPos);
            }
        }
    }
}
...
void cDrawSceneSimChar::ApplyUIForce(double time_step)
{
    if (ObjectSelected())
    {
        ...
        tPerturb perturb = tPerturb(tPerturb::ePerturbForce, mSelectedObj,
                                    mSelectObjLocalPos, force, time_step);
...
void cDrawSceneSimChar::DrawCoM() const
{
    ...
    for (int i = 0; i < mScene->GetNumChars(); ++i)
    {
        ...
        cDrawSimCharacter::DrawCoM(*(Downcast(character).get()), marker_size,
                                   vel_scale, col, GetVisOffset());
    }
}
...
void cDrawSceneSimChar::DrawTorque() const
{
    ...
    {
        const auto &character = mScene->GetCharacter(i);
        cDrawSimCharacter::DrawTorque(*(Downcast(character).get()),
                                      GetVisOffset());
    }
}
...
void cDrawSceneSimChar::DrawBodyVel() const
{
    ...
    {
        const auto &character = mScene->GetCharacter(i);
        cDrawSimCharacter::DrawBodyVel(*Downcast(character).get(),
...
void cDrawSceneSimChar::DrawHeading() const
{
    for (int i = 0; i < mScene->GetNumChars(); ++i)
    {
        ...
        cDrawCharacter::DrawHeading(*(character.get()), arrow_size, arrow_col,
                                    GetVisOffset());
    }
}
...
void cDrawSceneSimChar::DrawCharacter(
    const std::shared_ptr<cSimCharacterBase> &character) const
{
    const tVector fill_tint = tVector(1, 1, 1, 1);
    bool enable_draw_shape = true;
    cDrawSimCharacter::Draw(*(character.get()), fill_tint, GetLineColor(),
                            enable_draw_shape);
}
...
void cDrawSceneSimChar::DrawInfo() const { DrawPoliInfo(); }

void cDrawSceneSimChar::DrawPoliInfo() const
{
    const auto &character = mScene->GetCharacter();
    const cDeepMimicCharController *trl_ctrl =
        dynamic_cast<cDeepMimicCharController *>(
