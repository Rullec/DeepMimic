...
const tVector gVisOffset =
    tVector(0, 0, 1, 0); // offset for visualization elements
...
void cDrawScene::EnableDrawInfo(bool enable) { mDrawInfo = enable; }

std::string cDrawScene::GetName() const { return "Draw"; }

void cDrawScene::ParseCamTrackMode(const std::shared_ptr<cArgParser> &parser,
                                   eCamTrackMode &out_mode) const
{
    std::string str = "";
...
void cDrawScene::UpdateCameraStill()
{
    ...
    double avg_h = 0;
    bool snap_x =
        std::abs(track_pos[0] - cam_focus[0]) > cam_still_snap_dist - pad_x;
...
tVector cDrawScene::GetCamTrackPos() const { return tVector::Zero(); }

tVector cDrawScene::GetCamStillPos() const { return tVector::Zero(); }
...
tVector cDrawScene::GetDefaultCamFocus() const { return tVector::Zero(); }
...
tVector cDrawScene::GetBkgColor() const { return tVector(0.97, 0.97, 1, 0); }
...
void cDrawScene::DrawGroundMainScene()
{
    ...
    mShaderMesh->SetUniform4(mMeshMaterialDataHandle,
                             tVector(roughness, enable_tex, 0, 0));
...
tVector cDrawScene::GetVisOffset() const { return tVector::Zero(); }

tVector cDrawScene::GetLineColor() const { return gLineColor; }

tVector cDrawScene::GetGroundColor() const { return tVector::Ones(); }
...
void cDrawScene::InitRenderResources()
{
...

    {
        ...
        succ &= mShaderDepth->BuildShader("data/shaders/Mesh_VS.glsl",
                                          "data/shaders/Depth_PS.glsl");
    }

    {
        mShaderMesh = std::unique_ptr<cShader>(new cShader());
        succ &=
            mShaderMesh->BuildShader("data/shaders/Mesh_VS.glsl",
                                     "data/shaders/Lighting_Lambert_PS.glsl");
        ...
        mShaderMesh->GetUniformHandle(mMeshAmbientColourHandle,
                                      "gAmbientColour");
        ...
        GLint albedo_tex =
            glGetUniformLocation(mShaderMesh->GetProg(), "gTexture");
        glUniform1i(albedo_tex, 0);
        GLint shadow_tex =
            glGetUniformLocation(mShaderMesh->GetProg(), "gShadowTex");
    ...
    int shadow_res = 2048;
    mShadowCam = cCamera(cCamera::eProjOrtho, tVector(0, 0, 1, 0),
                         tVector::Zero(), tVector(0, 1, 0, 0), shadow_size,
                         shadow_size, shadow_near_z, shadow_far_z);
...
{
    bool succ = true;
    mGridTex = std::unique_ptr<cTextureDesc>(
        new cTextureDesc("data/textures/grid0.png", true));
...
void cDrawScene::SetupMeshShader()
{
    ...
    glProgramUniformMatrix4fv(mShaderMesh->GetProg(), mMeshShadowProjHandle, 1,
