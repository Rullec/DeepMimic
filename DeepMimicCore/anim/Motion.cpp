...
const std::string gLoopStr[cMotion::eLoopMax] = {"none", "wrap", "mirror"};
...
bool cMotion::Load(const tParams &params)
{
    ...
    {
        MIMIC_ERROR("Failed to parse Json from {}", mParams.mMotionFile)
    }
    return succ;
}
...
int cMotion::GetNumDof() const { return GetFrameSize() - eFrameMax; }
...
void cMotion::BuildFrameVel(Eigen::MatrixXd &out_frame_vel,
                            bool mirror /*= false*/) const
{
    ...
    // std::cout <<"cMotion::BuildFrameVel end " << num_frames <<" " << dof <<
    // std::endl;
...
void cMotion::CalcFrame(double time, tFrame &out_frame,
                        bool force_mirror /*=false*/) const
{
    int idx;
    double phase;
...
void cMotion::CalcFrameVel(double time, cMotion::tFrame &out_vel,
                           bool force_mirror /*=false*/) const
{
...
void cMotion::CalcFramePhase(double phase, tFrame &out_frame,
                             bool force_mirror /*=false*/) const
{
...
bool cMotion::LoadJson(const Json::Value &root)
{
    ...
    mVelFilterCutoff =
        root.get(gVelFilterCutoffKey, mVelFilterCutoff).asDouble();
...
bool cMotion::LoadJsonFrames(const Json::Value &root,
                             Eigen::MatrixXd &out_frames) const
{
    bool succ = true;
    ...
    if (num_frames > 0)
    {
        ...
        // std::cout <<"[log] cMotion data resize = " << num_frames <<" " <<
        // data_size << std::endl;
...
bool cMotion::ParseFrameJson(const Json::Value &root,
                             Eigen::VectorXd &out_frame) const
{
    bool succ = false;
    if (root.isArray())
    {
...
bool cMotion::LoadJsonJoints(const Json::Value &root,
                             std::vector<int> &out_right_joints,
                             std::vector<int> &out_left_joints) const
{
    bool succ = true;
...
std::string cMotion::BuildLoopStr(eLoop loop) const { return gLoopStr[loop]; }
...
void cMotion::PostProcessFrames(Eigen::MatrixXd &frames) const
{
    ...
    for (int f = 0; f < num_frames; ++f)
    {
        ...
        {
            Eigen::VectorXd pose =
                curr_frame.segment(eFrameMax, frame_size - eFrameMax);
...
bool cMotion::AddFrame(const tFrame &frame, double timestep)
{
    ...
    */
    ...
    // std::cout << "mframe shape = " << mFrames.rows() <<" " << mFrames.cols()
    // << std::endl; std::cout <<"fr = " << frame.size() << std::endl; std::cout
    // <<"set timestep = " << timestep << std::endl;

    mFrames(mFrames.rows() - 1, 0) = timestep;
    mFrames.row(mFrames.rows() - 1).segment(1, frame.size()) =
        frame.transpose();
...
void cMotion::FinishAddFrame() { PostProcessFrames(mFrames); }
...
void cMotion::CalcIndexPhase(double time, int &out_idx, double &out_phase) const
{
    ...
    auto it = std::upper_bound(
        frame_times.data(), frame_times.data() + frame_times.size(),
        time); // ���ش���time�ĵ�һ��Ԫ�صĵ�ַ
    out_idx =
        static_cast<int>(it - frame_times.data() - 1); // the first element is
    double time0 = frame_times(out_idx);
    double time1 = frame_times(out_idx + 1);
    out_phase =
        (time - time0) /
        (time1 - time0); // ����һ֡�ߵ�ʲô�׶��ˣ���
}
...
bool cMotion::HasBlendFunc() const { return mParams.mBlendFunc != nullptr; }

bool cMotion::HasMirrorFunc() const { return mParams.mMirrorFunc != nullptr; }

bool cMotion::HasVelFunc() const { return mParams.mVelFunc != nullptr; }
...
bool cMotion::EnableLoop() const { return mLoop != eLoopNone; }

cMotion::eLoop cMotion::GetLoop() const { return mLoop; }

void cMotion::SetLoop(eLoop loop) { mLoop = loop; }

void cMotion::BlendFramesIntern(const Eigen::VectorXd *a,
                                const Eigen::VectorXd *b, double lerp,
                                cMotion::tFrame *out_frame) const
{
...
void cMotion::MirrorFrame(tFrame &out_frame) const
{
    ...
    mParams.mMirrorFunc(&mParams.mLeftJoints, &mParams.mRightJoints,
                        &out_frame);
}

void cMotion::CalcFrameVel(const tFrame &frame0, const tFrame &frame1,
...
void cMotion::Output(const std::string &out_filepath) const
{
    ...
    if (nullptr == file)
    {
        std::cout << "[error] cMotion::Output(const std::string& out_filepath) "
                     "const: illegal motion path "
