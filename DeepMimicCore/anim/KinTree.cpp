...
bool cKinTree::LoadBodyDefs(const std::string &char_file,
                            ...
                            std::vector<std::string> &out_body_names)
{
    bool succ = true;
    std::string str;
    ...
    if (succ)
    {
        ...
        if (!root[gBodyDefsKey].isNull())
        {
            ...
            for (int b = 0; b < num_bodies; ++b)
            {
                ...
                bool succ_def =
                    ParseBodyDef(body_json, curr_def, out_body_names[b]);
...
bool cKinTree::ParseBodyDef(const Json::Value &root,
                            cKinTree::tBodyDef &out_def, std::string &out_name)
{
    ...
    {
        std::cout
            << "[error] cKinTree::ParseBodyDef: parse body def name failed"
            << std::endl;
        exit(1);
    }
    else
...
bool cKinTree::LoadDrawShapeDefs(const std::string &char_file,
                                 ...
                                 std::vector<std::string> &out_draw_names)
{
    bool succ = true;
    std::string str;
    ...
    if (succ)
    {
        if (!root[gDrawShapeDefsKey].isNull())
        {
            ...
            for (int b = 0; b < num_shapes; ++b)
            {
                ...
                bool succ_def =
                    ParseDrawShapeDef(shape_json, curr_def, out_draw_names[b]);
...
bool cKinTree::ParseDrawShapeDef(const Json::Value &root,
                                 tDrawShapeDef &out_def, std::string &out_name)
{
    std::string shape_str = root.get(gDrawShapeDescKeys[eDrawShapeShape], "")
                                .asString(); // "Sphere", "box" etc
    ...
    {
        printf("[error] cKinTree::ParseDrawShapeDef: load %s failed",
               gDrawShapeDefsKey.c_str());
        succ &= false;
    }
    else
    {
        out_name = root[gDrawShapeDefsNameKey].asString();
        // std::cout << "[debug] load draw shape name = " << out_name <<
        // std::endl;
    }
...
cShape::eShape cKinTree::GetBodyShape(const Eigen::MatrixXd &body_defs,
                                      int part_id)
{
    // bodyshape是从bodydefs来的，和json文件中的第二个部分相对应
...
{
    const cKinTree::tBodyDef &def =
        body_defs.row(part_id); // 这是一个存储结构，好像从json中读取过来的。
    // 所以说BodyDefs里面的AttachXYZ是当前mesh(body)和父亲mesh(body)的连接点在local坐标系下的坐标
    // 我靠，我本该想到的，Attach是连接啊...
                                     ...
                                     int part_id)
{
    tVector attach_theta =
        tVector(body_defs(part_id, eBodyParamAttachThetaX),
                                        ...
                                        int part_id)
{
    ...
    return fall_val != 0; // 如果是0，返回假，所以脚应该是0
}

void cKinTree::SetBodyEnableFallContact(int part_id, bool enable,
                                        Eigen::MatrixXd &out_body_defs)
{
    // std::cout <<"set " << part_id <<"th body contact = " << enable <<
    // std::endl;
...
bool cKinTree::Load(const Json::Value &root, Eigen::MatrixXd &out_joint_mat,
                    std::vector<std::string> &out_joint_name)
{
    ...
    if (!root[gJointsKey].isNull())
    {
        ...
        for (int j = 0; j < num_joints; ++j)
        {
            ...
            // 下面这段只是在检查是否存在非法的parent id > child
            // id的情况，没干别的。
            const auto &curr_desc = out_joint_mat.row(j);
            int parent_id =
                static_cast<int>(curr_desc(eJointDescParent)); // 获得parent id
            if (parent_id >= j)
            {
                // 这里面存在Parent id < child id的要求
                printf("Parent id must be < child id, parent id: %i, child id: "
                       "%i\n",
                       parent_id, j);
...
{
    cSpAlg::tSpVec sv =
        cRBDUtil::CalcWorldVel(joint_mat, state, vel, parent_id);
    tVector pos =
...
tVector cKinTree::CalcJointWorldAngularVel(const Eigen::MatrixXd &joint_mat,
                                           ...
                                           int joint_id)
{
    return CalcWorldAngularVel(joint_mat, state, vel, joint_id,
                               tVector::Zero());
}
...
void cKinTree::GetChild(const Eigen::MatrixXd &joint_mat, int parent_id,
                          ...
                          std::string &out_joint_name)
{
    ...
    {
        printf(
            "[log] cKinTree:ParseJoint: No Skeleton-Joints Name specified\n");
        exit(1);
    }
    else
    {
        out_joint_name = name_json.asString();
        // std::cout << "[debug] cKinTree:ParseJoint: read json name = " <<
        // out_joint_name << std::endl;
    }
...
{
    // 对joint进行后处理,之前已经找好父子节点关系了。
    int num_joints = GetNumJoints(out_joint_mat);
    int offset = 0;
    for (int j = 0; j < num_joints; ++j)
    {
        int curr_size = GetParamSize(out_joint_mat, j);
        // 这个好像只是设置offset，用来更方便的找到joint的参数
        // 也就是说load函数根本没有重建全局坐标，只是读了一下json而已。
                                   ...
                                   int joint_id)
{
    ...
    tMatrix mat = cMathUtil::RotateMat(attach_theta, eRotationOrder::XYZ);
    mat(0, 3) = attach_pt[0];
    mat(1, 3) = attach_pt[1];
    mat(2, 3) = attach_pt[2];
    return mat;
...
tMatrix cKinTree::BodyJointTrans(const Eigen::MatrixXd &body_defs, int part_id)
{
    ...
    tMatrix rot = cMathUtil::RotateMat(euler, eRotationOrder::XYZ);
    tMatrix trans = cMathUtil::TranslateMat(attach_pt + com);
    tMatrix m = trans * rot;
    return m; // body frame to parent joint frame
}
                                 ...
                                 Eigen::VectorXd &out_diff)
{
    ...
    if (is_root)
    {
        ...
        out_diff.segment(gPosDim, gRotDim) =
            cMathUtil::QuatToVec(root_rot_diff);
    }
    else
    {
        eJointType joint_type = GetJointType(joint_mat, joint_id);
                       ...
                       Eigen::VectorXd &out_vel)
{
    ...
    tVector root_rot_vel =
        cMathUtil::CalcQuaternionVel(root_rot0, root_rot1, dt);
                         ...
                         Eigen::VectorXd &out_pose)
{
    ...
    for (int j = 1; j < num_joints; ++j)
    {
        ...
        {
            tQuaternion rot0 =
                cMathUtil::VecToQuat(pose0.segment(offset, size));
            tQuaternion rot1 =
                cMathUtil::VecToQuat(pose1.segment(offset, size));
                             ...
                             Eigen::VectorXd &out_pose_diff)
{
    ...
    tVector root_rot_vel = GetRootAngVel(joint_mat, vel); // 格式?
    root_rot_vel[3] = 0;
    tQuaternion root_rot = GetRootRot(joint_mat, pose);
    tMatrix root_diff_mat = cMathUtil::BuildQuaternionDiffMat(root_rot);
    tVector root_diff = root_diff_mat * root_rot_vel;        // q' = 1/2 * w * q
    tQuaternion root_quat = cMathUtil::VecToQuat(root_diff); // rotation matrix

    // set root quaternion diff dqdt's coef [w, x, y, z] to out_pose_diff
    ...
    for (int j = 1; j < num_joints; ++j)
    {
        ...
        if (joint_type == eJointTypeSpherical)
        {
            ...
            // if it is spherical, set q' coeff to out_pose_diff
                             ...
                             const Eigen::VectorXd &pose)
{
    ...
    // 其实就是root坐标系转动多少能够在xoz上和世界坐标系平齐
    ...
    double heading = std::atan2(
        -rot_dir[2], rot_dir[0]); // y = -rot_dir[2], x = rot_dir[0],计算反三角
...
/**
 * \brief			calculate the rotation matrix which can rotate
 * the orientation of the skeleton to be parallel to the vector (0, 0, 1)
 */
 ...
 * This function will return a transform matrix 4x4. It can convert a point in
 * world frame into root local frame
 */
                                   ...
                                   const Eigen::VectorXd &pose)
{
    ...
    // with x-axis aligned along the character's heading
    tVector origin = GetRootPos(
        joint_mat,
        pose); // 获取世界坐标系下root joint位置在xoz平面上的投影，称为origin:
               // 也就是从joint坐标系到世界坐标系的变换
    origin[1] = 0; // 例如此时origin = [0, 0.89, 0], 之后就是[0, 0, 0],
                   // 相当于抹平了Y轴变换
    tMatrix rot_mat = BuildHeadingTrans(
        joint_mat, pose); // rotate the orientation of the skeleton to be
                          // parallel with vector (0, 0, 1)
    tMatrix trans_mat =
...
void cKinTree::MirrorPoseStance(const Eigen::MatrixXd &joint_mat,
                                const std::vector<int> mirror_joints0,
                                const std::vector<int> mirror_joints1,
                                Eigen::VectorXd &out_pose)
{
                                           ...
                                           int joint_id)
{
    ...
    {
        R = cMathUtil::RotateMat(
            tVector(0, 0, 1, 0),
            theta); // here is deepmimic order. z is the axis of revolute joint
    }
    else
    {
        R = cMathUtil::RotateMat(tVector(1, 0, 0, 0), theta); // here
        // is robotmodel order. x is the axis of revolute joint
    }
                                            ...
                                            int joint_id)
{
    ...
    tQuaternion q =
        cMathUtil::VecToQuat(state.segment(param_offset, param_size));
