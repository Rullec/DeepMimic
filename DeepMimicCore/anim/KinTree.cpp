...
{
	const cKinTree::tBodyDef& def = body_defs.row(part_id);	// 这是一个存储结构，好像从json中读取过来的。
...
void cKinTree::GetBodyRotation(const Eigen::MatrixXd& body_defs, int part_id, tVector& out_axis, double& out_theta)
{
	...
	cMathUtil::EulerToAxisAngle(theta, out_axis, out_theta, eRotationOrder::XYZ);
}
...
bool cKinTree::Load(const Json::Value& root, Eigen::MatrixXd& out_joint_mat, std::vector<std::string> &out_joint_name)
{
	...
	if (!root[gJointsKey].isNull())
	{
        ...
        for (int j = 0; j < num_joints; ++j)
		{
			...
			// 下面这段只是在检查是否存在非法的parent id > child id的情况，没干别的。
			const auto& curr_desc = out_joint_mat.row(j);
			int parent_id = static_cast<int>(curr_desc(eJointDescParent));// 获得parent id
			if (parent_id >= j)
			{
...
void cKinTree::SetRootRot(const Eigen::MatrixXd& joint_mat, const tQuaternion& rot,\
...
void cKinTree::CalcVel(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose0,
	const Eigen::VectorXd& pose1, double dt, Eigen::VectorXd& out_vel)
{
...
tMatrix cKinTree::ChildParentTransRevolute(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& state, int joint_id)
{
	...
	tMatrix A = BuildAttachTrans(joint_mat, joint_id);
	tMatrix R = cMathUtil::RotateMat(tVector(1, 0, 0, 0), theta);
