...
void cKinCharacter::Reset() { cCharacter::Reset(); }
...
bool cKinCharacter::LoadMotion(const std::string &motion_file)
{
    ...
    motion_params.mBlendFunc = std::bind(
        &cKinCharacter::BlendFrames, this, std::placeholders::_1,
        std::placeholders::_2, std::placeholders::_3, std::placeholders::_4);
    motion_params.mMirrorFunc =
        std::bind(&cKinCharacter::MirrorFrame, this, std::placeholders::_1,
                  std::placeholders::_2, std::placeholders::_3);
    motion_params.mVelFunc = std::bind(
        &cKinCharacter::CalcFrameVel, this, std::placeholders::_1,
        std::placeholders::_2, std::placeholders::_3, std::placeholders::_4);
    motion_params.mPostProcessFunc = std::bind(&cKinCharacter::PostProcessFrame,
...
const cMotion &cKinCharacter::GetMotion() const { return mMotion; }
...
double cKinCharacter::GetTime() const { return mTime; }
...
// the concept of "phase" means how many cycle has this character achieved
// acoording to the ref motion.
...
bool cKinCharacter::HasMotion() const { return mMotion.IsValid(); }

/**
 * \brief					Set Root Pos to a specified
 * place pos \param					pos, new root position
 */
void cKinCharacter::SetRootPos(const tVector &pos)
{
    ...
    // 2. calculate the vector from current root pos to ideal root position
    // "pos".
...
const tVector &cKinCharacter::GetOriginPos() const { return mOrigin; }

// set mOrigin position. mOrigin is the root point of this motion. It will not
// change during simulating, but only changed by set value directly or
// Reset...etc
...
const tQuaternion &cKinCharacter::GetOriginRot() const { return mOriginRot; }
...
{
    // std::cout <<"------------begin to calculate pose for time " << time <<
    // std::endl; given a time, how to compute the pose accordly
    tVector root_delta = tVector::Zero(); // root delta translation is zero
    tQuaternion root_delta_rot =
        tQuaternion::Identity(); // root delta rotation is identity
    ...
    if (HasMotion())
    {
        ...
        // std::cout <<"void cKinCharacter::CalcPose: pose size = " <<
        // out_pose.size() << std::endl; std::cout <<"void
        // cKinCharacter::CalcPose: pose = " << out_pose.transpose() <<
        // std::endl;
        if (mMotion.EnableLoop())
        {
    ...
    // 2. fetch the root_pos and root_rot in out_pose, which is purely ref
    // motion
    ...
    // std::cout <<"------------end to calculate pose for time " << time <<
    // std::endl;
}
...
void cKinCharacter::BlendFrames(const cMotion::tFrame *a,
                                const cMotion::tFrame *b, double lerp,
                                cMotion::tFrame *out_frame) const
{
    cKinTree::LerpPoses(mJointMat, *a, *b, lerp, *out_frame);
}

void cKinCharacter::MirrorFrame(const std::vector<int> *right_joints,
                                const std::vector<int> *left_joints,
                                cMotion::tFrame *out_frame) const
{
    const auto &joint_mat = GetJointMat();
    cKinTree::MirrorPoseStance(joint_mat, *left_joints, *right_joints,
                               *out_frame);
}

void cKinCharacter::CalcFrameVel(const cMotion::tFrame *a,
                                 const cMotion::tFrame *b, double timestep,
                                 cMotion::tFrame *out_vel) const
{
    const auto &joint_mat = GetJointMat();
