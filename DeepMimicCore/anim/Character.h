...
public:
    ...
    virtual double CalcJointChainLength(int joint_id) const;
    ...
    virtual bool WriteState(const std::string &file,
...
protected:
    ...
    virtual bool ParseState(const Json::Value &root,
                            Eigen::VectorXd &out_state) const;
    virtual std::string BuildStateJson(const Eigen::VectorXd &pose,
                                       const Eigen::VectorXd &vel) const;

    virtual bool LoadDrawShapeDefs(const std::string &char_file,
                                   Eigen::MatrixXd &out_draw_defs);
    virtual bool
    LoadMeshes(const std::string &char_file,
