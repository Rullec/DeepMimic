...
const std::string cBVHReader::gReserved[] = {"HIERARCHY", "ROOT", "OFFSET",
                                             "CHANNELS", "MOTION"};
const std::string cBVHReader::gChannelNames[] = {"Xposition", "Yposition",
                                                 "Zposition", "Zrotation",
                                                 "Xrotation", "Yrotation"};
...
int cBVHReader::parseJoint(std::istream &stream, int parent_id)
{
    ...
    while (stream.good())
    {
        ...
        else if (tmp == "End")
        {
            ...
            if (tmp == "OFFSET")
            {
                stream >> end_joint.offset(0) >> end_joint.offset(1) >>
                    end_joint.offset(2);
...
tMatrix cBVHReader::getTransformationForFrame(const tJointData &joint,
                                              size_t frame_) const
{
    return this->getTransformationForFrame(
        joint.channel_start, joint.channels_order, motionData.data, frame_);
}

tMatrix cBVHReader::getTranslationForFrame(const tJointData &joint,
                                           size_t frame_) const
{
    return this->getTranslationForFrame(
        joint.channel_start, joint.channels_order, motionData.data, frame_);
}

tMatrix cBVHReader::getRotationForFrame(const tJointData &joint,
                                        size_t frame_) const
{
    return this->getRotationForFrame(joint.channel_start, joint.channels_order,
                                     motionData.data, frame_);
}
...
void cBVHReader::BuildJointMat(Eigen::MatrixXd &out_joint_mat) const
{
    ...
    for (int j = 0; j < num_joints; ++j)
    {
        ...
        curr_desc[cKinTree::eJointDescType] =
            static_cast<int>(joint.mJointType);
...
tMatrix cBVHReader::getTransformationForFrame(
    size_t channel_start, const std::vector<Channels> &channels_order,
    const Eigen::MatrixXd &frame_data, size_t frame) const
{
    tMatrix trans_mat = getTranslationForFrame(channel_start, channels_order,
                                               frame_data, frame);
    tMatrix rot_mat =
        getRotationForFrame(channel_start, channels_order, frame_data, frame);
    tMatrix mat = trans_mat * rot_mat;
    return mat;
}

tMatrix cBVHReader::getTranslationForFrame(
    size_t channel_start, const std::vector<Channels> &channels_order,
    const Eigen::MatrixXd &frame_data, size_t frame) const
{
...
tMatrix cBVHReader::getRotationForFrame(
    size_t channel_start, const std::vector<Channels> &channels_order,
    const Eigen::MatrixXd &frame_data, size_t frame) const
{
    tMatrix mat = tMatrix::Identity();
...
bool cBVHReader::GetJointLocation(std::string jointName, size_t frame_,
                                  tVector &position, tMatrix &transform) const
{
...
bool cBVHReader::GetJointLocation(const tJointData &joint,
                                  std::string jointName, size_t frame_,
                                  tVector &position, tMatrix &transform) const
{
    tMatrix transJoint =
        joint.matrix * getTransformationForFrame(joint.channel_start,
                                                 joint.channels_order,
                                                 motionData.data, frame_);
    ...
    for (size_t j = 0; j < joint.children.size(); j++)
    {
        ...
        bool found =
            GetJointLocation(child_data, jointName, frame_, position, transTMP);
        if (found)
        {
            transform = transTMP;
            return found;
...
cKinTree::eJointType
cBVHReader::CalcJointType(const tJointData &joint_data) const
{
...
void cBVHReader::BuildJointPose(int j, int frame,
...
void cBVHReader::BuildJointPoseRoot(int j, int frame,
                                    Eigen::VectorXd &out_pose) const
{
    const tJointData &joint = GetJointData(j);
    ...
    out_pose.segment(0, cKinTree::gPosDim) =
        offset.segment(0, cKinTree::gPosDim);
    out_pose.segment(cKinTree::gPosDim, cKinTree::gRotDim) =
        cMathUtil::QuatToVec(quat);
}

void cBVHReader::BuildJointPoseRevolute(int j, int frame,
                                        Eigen::VectorXd &out_pose) const
{
    const tJointData &joint = GetJointData(j);
...
void cBVHReader::BuildJointPosePlanar(int j, int frame,
                                      Eigen::VectorXd &out_pose) const
{
    const tJointData &joint = GetJointData(j);
    int param_size = cKinTree::GetJointParamSize(joint.mJointType);
...
void cBVHReader::BuildJointPosePrismatic(int j, int frame,
                                         Eigen::VectorXd &out_pose) const
{
    const tJointData &joint = GetJointData(j);
    int param_size = cKinTree::GetJointParamSize(joint.mJointType);
...
void cBVHReader::BuildJointPoseFixed(int j, int frame,
                                     Eigen::VectorXd &out_pose) const
{
    const tJointData &joint = GetJointData(j);
    int param_size = cKinTree::GetJointParamSize(joint.mJointType);
...
void cBVHReader::BuildJointPoseSpherical(int j, int frame,
                                         Eigen::VectorXd &out_pose) const
{
    const tJointData &joint = GetJointData(j);
    int param_size = cKinTree::GetJointParamSize(joint.mJointType);
