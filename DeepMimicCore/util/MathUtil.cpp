...
tMatrix cMathUtil::RotateMat(const tVector& euler, const eRotationOrder gRotationOrder)
{
	double x = euler[0];
	double y = euler[1];
	double z = euler[2];
...
tVector cMathUtil::InvEuler(const tVector& euler, const eRotationOrder gRotationOrder)
{
	if (gRotationOrder == eRotationOrder::XYZ)
	{
		...
		tVector inv_euler = cMathUtil::RotMatToEuler(inv_mat, eRotationOrder::XYZ);
		return inv_euler;
	}
	else
	{
...
tVector cMathUtil::RotMatToEuler(const tMatrix& mat, const eRotationOrder gRotationOrder)
{
	tVector euler;
	if (gRotationOrder == eRotationOrder::XYZ)
	{
...
void cMathUtil::EulerToAxisAngle(const tVector& euler, tVector& out_axis, double& out_theta, const eRotationOrder gRotationOrder)
{
...
tQuaternion cMathUtil::EulerToQuaternion(const tVector& euler, const eRotationOrder order)
{
	tVector axis;
	double theta;
...
tVector cMathUtil::QuaternionToEuler(const tQuaternion& q, const eRotationOrder gRotationOrder)
{
	if (gRotationOrder == eRotationOrder::XYZ)
	{
