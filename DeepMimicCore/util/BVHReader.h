...
public:
    ...
    struct PoseData
    {
        ...
        double frame_step = 0.0;              // The timestep of the recording
                                              // ordered list of channels
        std::vector<Channels> channels_types; // Used to find rotations and
    ...
    virtual Eigen::MatrixXd getMotoinData() const
    {
        return this->motionData.data;
    }
    virtual void SetModelTransform(const tMatrix &trans);

    // virtual void processMotionClips();
    /// Should return a cMotion, for now really only supports two step
    /// locations.

    virtual bool GetJointLocation(std::string jointName, size_t frame_,
                                  tVector &position, tMatrix &transform) const;
    virtual bool GetJointLocation(const tJointData &joint,
                                  std::string jointName, size_t frame_,
                                  tVector &position, tMatrix &transform) const;

    virtual tMatrix getTransformationForFrame(const tJointData &joint,
                                              size_t frame) const;
    virtual tMatrix getTranslationForFrame(const tJointData &joint,
                                           size_t frame) const;
    virtual tMatrix getRotationForFrame(const tJointData &joint,
                                        size_t frame) const;
    ...
    virtual void BuildMotion(double target_framerate,
                             cMotion &out_motion) const;
...
private:
    ...
    virtual tMatrix getTransformationForFrame(
        size_t channel_start, const std::vector<Channels> &channels_order,
        const Eigen::MatrixXd &frame_data, size_t frame) const;
    virtual tMatrix getTranslationForFrame(
        size_t channel_start, const std::vector<Channels> &channels_order,
        const Eigen::MatrixXd &frame_data, size_t frame) const;
    virtual tMatrix
    getRotationForFrame(size_t channel_start,
                        const std::vector<Channels> &channels_order,
                        const Eigen::MatrixXd &frame_data, size_t frame) const;
    ...
    virtual cKinTree::eJointType
    CalcJointType(const tJointData &joint_data) const;
    virtual void ConvertFrameToPose(int f, Eigen::VectorXd &out_pose) const;

    virtual void BuildJointPose(int j, int frame,
                                Eigen::VectorXd &out_pose) const;
    virtual void BuildJointPoseRoot(int j, int frame,
                                    Eigen::VectorXd &out_pose) const;
    virtual void BuildJointPoseRevolute(int j, int frame,
                                        Eigen::VectorXd &out_pose) const;
    virtual void BuildJointPosePlanar(int j, int frame,
                                      Eigen::VectorXd &out_pose) const;
    virtual void BuildJointPosePrismatic(int j, int frame,
                                         Eigen::VectorXd &out_pose) const;
    virtual void BuildJointPoseFixed(int j, int frame,
                                     Eigen::VectorXd &out_pose) const;
    virtual void BuildJointPoseSpherical(int j, int frame,
                                         Eigen::VectorXd &out_pose) const;
};
