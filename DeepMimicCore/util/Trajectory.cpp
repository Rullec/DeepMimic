...
void cTrajectory::Clear() { mAnchors.resize(0, 0); }
...
int cTrajectory::GetDim() const { return static_cast<int>(mAnchors.rows()); }

int cTrajectory::GetSize() const { return static_cast<int>(mAnchors.size()); }
...
void cTrajectory::Compress(double ratio)
{
    Filter(ratio / 2);
    int num_anchors = GetNumAnchors();
    int num_compressed_anchors =
        static_cast<int>(std::ceil(num_anchors * ratio));
    Eigen::MatrixXd new_anchors =
        Eigen::MatrixXd(GetDim(), num_compressed_anchors);
...
Eigen::VectorXd cTrajectory::Unroll() const
{
    ...
    // Eigen::VectorXd unrolled = Eigen::VectorXd::Map(mAnchors.data(),
    // mAnchors.size());
    return unrolled;
}
