                                    ...
                                    const std::string &path)
{
    ...
    case eTrajFileVersion::V1:
        LoadTrajV1(load_info, path);
        break;
    case eTrajFileVersion::V2:
        LoadTrajV2(load_info, path);
        break;
    }
}
 ...
 */
void cInteractiveIDSolver::LoadTrajV1(tLoadInfo &load_info,
                                      const std::string &path)
{
    ...
    int num_of_frames = list_json.size();
    {
        ...
        load_info.mPoseMat.resize(num_of_frames, mDof),
            load_info.mPoseMat.setZero();
        load_info.mVelMat.resize(num_of_frames, mDof),
            load_info.mVelMat.setZero();
        load_info.mAccelMat.resize(num_of_frames, mDof),
            load_info.mAccelMat.setZero();
        load_info.mActionMat.resize(num_of_frames,
                                    mCharController->GetActionSize()),
            load_info.mActionMat.setZero();
        load_info.mPDTargetMat.resize(num_of_frames,
                                      mCharController->GetActionSize()),
            load_info.mPDTargetMat.setZero();
        load_info.mContactForces.resize(num_of_frames);
        for (auto &x : load_info.mContactForces)
            x.clear();
        load_info.mLinkRot.resize(num_of_frames);
        for (auto &x : load_info.mLinkRot)
            x.resize(mNumLinks);
        load_info.mLinkPos.resize(num_of_frames);
        for (auto &x : load_info.mLinkPos)
            x.resize(mNumLinks);
        load_info.mExternalForces.resize(num_of_frames);
        for (auto &x : load_info.mExternalForces)
            x.resize(mNumLinks);
        load_info.mExternalTorques.resize(num_of_frames);
        for (auto &x : load_info.mExternalTorques)
            x.resize(mNumLinks);
        load_info.mTruthJointForces.resize(num_of_frames);
        for (auto &x : load_info.mTruthJointForces)
            x.resize(mNumLinks - 1);
        load_info.mTimesteps.resize(num_of_frames),
            load_info.mTimesteps.setZero();
        load_info.mRewards.resize(num_of_frames), load_info.mRewards.setZero();
        load_info.mMotionRefTime.resize(num_of_frames),
            load_info.mMotionRefTime.setZero();
    }
    ...
    for (int frame_id = 0; frame_id < num_of_frames; frame_id++)
    {
        ...
        if (frame_id >= 1)
            assert(cur_vel.size() == mDof);
        assert(cur_accel.isNull() == false);
        if (frame_id >= 2)
            assert(cur_accel.size() == mDof);
        ...
        assert(cur_truth_joint_force.isNull() == false &&
               cur_truth_joint_force.size() == (mNumLinks - 1) * 4);
        // std::cout <<"load pd target size = " << cur_truth_action.size()
        // << std::endl; std::cout <<"action space size = " <<
        // mCharController->GetActionSize() << std::endl;
        assert(cur_truth_action.isNull() == false &&
               cur_truth_action.size() == mCharController->GetActionSize());
        assert(cur_truth_pd_target.isNull() == false &&
               cur_truth_pd_target.size() == mCharController->GetActionSize());

        // 1. pos, vel, accel
        for (int j = 0; j < mDof; j++)
            load_info.mPoseMat(frame_id, j) = cur_pose[j].asDouble();
        // std::cout <<cur_pose.size() <<" " << mSimChar->GetNumDof() <<
        // std::endl;
        SetGeneralizedPos(load_info.mPoseMat.row(frame_id));
        RecordMultibodyInfo(load_info.mLinkRot[frame_id],
                            load_info.mLinkPos[frame_id]);
        for (int j = 0; j < mDof && frame_id >= 1; j++)
            load_info.mVelMat(frame_id, j) = cur_vel[j].asDouble();
        for (int j = 0; j < mDof && frame_id >= 1; j++)
            load_info.mAccelMat(frame_id, j) = cur_accel[j].asDouble();
        ...
        for (int c_id = 0; c_id < cur_contact_info.size(); c_id++)
        {
           ...
           single_contact["force_value"].append(force.mForce[i]);
            single_contact["force_link_id"] = force.mId;
                */
                load_info.mContactForces[frame_id][c_id].mPos[i] =
                    cur_contact_info[c_id]["force_pos"][i].asDouble();
                load_info.mContactForces[frame_id][c_id].mForce[i] =
                    cur_contact_info[c_id]["force_value"][i].asDouble();
            }
            load_info.mContactForces[frame_id][c_id].mId =
                cur_contact_info[c_id]["force_link_id"].asInt();
            load_info.mContactForces[frame_id][c_id].mIsSelfCollision =
        ...
        for (int idx = 0; idx < mNumLinks; idx++)
        {
            // auto & cur_ext_force = ;
            // auto & cur_ext_torque =
            // load_info.mExternalTorques[frame_id][idx];
            for (int i = 0; i < 4; i++)
            {
                load_info.mExternalForces[frame_id][idx][i] =
                    cur_ext_force[idx * 4 + i].asDouble();
                load_info.mExternalTorques[frame_id][idx][i] =
        ...
        {
            for (int j = 0; j < 4; j++)
                load_info.mTruthJointForces[frame_id][idx][j] =
        ...
        {
            load_info.mActionMat(frame_id, idx) =
                cur_truth_action[idx].asDouble();
            load_info.mPDTargetMat(frame_id, idx) =
                cur_truth_pd_target[idx].asDouble();
        }
    }
#ifdef VERBOSE
...
void cInteractiveIDSolver::LoadTrajV2(tLoadInfo &load_info,
                                      const std::string &path)
{
    ...
    int pose_size = mSimChar->GetPose().size();
    {
        ...
        load_info.mAccelMat.resize(num_of_frames, mDof),
            load_info.mAccelMat.setZero();
        load_info.mCharPoseMat.resize(num_of_frames, pose_size),
            load_info.mCharPoseMat.setZero();
        load_info.mActionMat.resize(num_of_frames,
                                    mCharController->GetActionSize()),
            load_info.mActionMat.setZero();
        load_info.mPDTargetMat.resize(num_of_frames,
    ...
    for (int frame_id = 0; frame_id < num_of_frames; frame_id++)
    {
        ...
        {
            mLogger->error("LoadTrajV2: char pose shape is empty or "
                           "invalid {} != {}",
        ...
        {
            mLogger->error(
                "LoadTrajV2: contact info is empty or invalid {} != {}",
                cur_contact_info.size(), cur_contact_num);
            exit(0);
        }

        for (int j = 0; j < pose_size; j++)
            load_info.mCharPoseMat(frame_id, j) = cur_char_pose[j].asDouble();
        // std::cout <<cur_pose.size() <<" " << mSimChar->GetNumDof() <<
        // std::endl;
        ...
        load_info.mTimesteps[frame_id] =
            cJsonUtil::ParseAsDouble("timestep", cur_frame);
        load_info.mRewards[frame_id] =
            cJsonUtil::ParseAsDouble("reward", cur_frame);
        load_info.mMotionRefTime[frame_id] =
