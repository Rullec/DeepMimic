...
void cOnlineIDSolver::PostSim()
{
    ...
    else if (mSolvingMode == eSolvingMode::POS)
    {
        if (mFrameId >= 2)
        {
            ...
            // fout <<"buffer u " << mFrameId -1 <<" = " <<
            // mBuffer_u[mFrameId - 1].transpose()<< std::endl; fout
            // <<"buffer u " << mFrameId <<" = " <<
            // mBuffer_u[mFrameId].transpose()<< std::endl; fout
            // <<"buffer u dot " << mFrameId-1 <<" = " <<
    ...
    if (true == mEnableExternalForce)
    {
        ...
        for (int ID_link_id = 0; ID_link_id < mNumLinks; ID_link_id++)
        {
            ...
            if (ID_link_id == 0)
            {
                ...
                mMultibody->addBaseForce(
                    cBulletUtil::tVectorTobtVector(external_force));
            }
            else
            {
                external_force = tVector::Random() * 10;
                // add external force in world frame
                mMultibody->addLinkForce(
                    multibody_link_id,
                    cBulletUtil::tVectorTobtVector(external_force));
            }
    ...
    if (true == mEnableExternalTorque)
    {
        ...
        for (int ID_link_id = 0; ID_link_id < mNumLinks; ID_link_id++)
        {
            ...
            if (0 == ID_link_id)
            {
                ...
                mMultibody->addBaseTorque(
                    cBulletUtil::tVectorTobtVector(external_torque));
            }
            else
            {
                external_torque = tVector::Random() * 10;
    ...
    const tEigenArr<tVector> &external_torques) const
{
    ...
    for (int id = 0; id < solved_joint_forces.size(); id++)
    {
                              ...
                              1e-5) == false)
        {
            ...
            std::cout << "[error] online ID solved error: for joint " << id
                      << " diff : solved = "
                      << solved_joint_forces[id].transpose()
                      << ", truth = " << mJointForces[id].transpose()
                      << std::endl;
        }
    }
    if (err < 1e-5)
        std::cout << "[log] frame " << frame_id
                  << " online ID solved accurately\n";
}