...
void cDisplayIDSolver::PostSim()
{
    ...
    else if (mLoadInfo.mLoadMode == eLoadMode::LOAD_TRAJ)
    {
        ...
        // fout_pt <<"frame " << mLoadInfo.mCurFrame << " contact pts num =
        // " << contact_pts.size() << std::endl; for(auto &x : contact_pts)
        // {
        //     std::string name = "";
        //     if(x.mId == 0) name = mMultibody->getBaseName();
        //     else name = std::string(mMultibody->getLink(x.mId
        //     -1).m_linkName); std::cout <<"name = " << name << std::endl;
        //     fout_pt << "link name : " << mSimChar->GetBodyName(x.mId -
        //     1)\
        //      << ", contact pt pos = " << x.mPos.transpose().segment(0, 3)
        //      << std::endl;
        // }
        ...
        //         rot =
        //         cBulletUtil::btQuaternionTotQuaternion(mMultibody->getBaseWorldTransform().getRotation()).coeffs();
        //     }
        //     else
        //     {
        //         int multibody_link_id = i - 1;
        //         auto & cur_trans =
        //         mMultibody->getLinkCollider(multibody_link_id)->getWorldTransform();
        //         rot =
        //         cBulletUtil::btQuaternionTotQuaternion(cur_trans.getRotation()).coeffs();
        //         pos =
        //         cBulletUtil::btVectorTotVector1(cur_trans.getOrigin());
        //     }

        //     fout_pos <<"link " << i <<" pos = " << pos.segment(0,
        //     3).transpose() << ", rot = " << rot.transpose() << std::endl;
        // }
    }
    else if (mLoadInfo.mLoadMode == eLoadMode::LOAD_MOTION)
    {
        ...
        const int &cur_frame =
            mLoadInfo.mCurFrame % mLoadInfo.mMotion->GetNumFrames();
        ...
        //     tQuaternion root_rot = cKinTree::GetRootRot(mJointMat,
        //     out_pose);
        ...
        // std::cout <<"[id] error root pos = " <<
        // mSimChar->GetRootPos().transpose() << std::endl; std::cout
        // <<"[id] error root rot = " <<
        // mSimChar->GetRootRotation().coeffs().transpose() << std::endl;
        // std::cout <<"[id] error pose = " << out_pose.transpose() <<
        // std::endl;
    }
    else
    {
        std::cout << "[error] cDisplayIDSolver::DisplaySet mode invalid: "
                  << mLoadInfo.mLoadMode << std::endl;
        exit(1);
    }
...
void cDisplayIDSolver::Parseconfig(const std::string &conf)
{
    ...
    // std::cout <<"void cDisplayIDSolver::ParseConfigDisplay(const Json::Value
    // & save_value)\n";
    const Json::Value &display_traj_path = display_value["display_traj_path"],
                      display_motion_path =
                          display_value["display_motion_path"];

    // mLoadInfo.mEnableOutputMotionInfo = enable_output_motion_info.asBool();
    mLoadInfo.mEnableOutputMotionInfo =
        cJsonUtil::ParseAsBool("enable_output_motion_info", display_value);
    mLoadInfo.mOutputMotionInfoPath =
        cJsonUtil::ParseAsString("output_motion_info_path", display_value);

    if (mLoadInfo.mEnableOutputMotionInfo)
        cFileUtil::ClearFile(mLoadInfo.mOutputMotionInfoPath);

    // there is only one choice between display_motion_path and
    // display_traj_path
    ...
    if (!display_motion_path_isnull || !display_traj_path_isnull)
    {
        ...
        {
            std::cout
                << "[error] cDisplayIDSolver::ParseConfigDisplay: there is only one choice between \
    ...
    {
        std::cout << "[error] cDisplayIDSolver::ParseConfigDisplay: all "
                     "options are empty\n";
        exit(1);
    }
