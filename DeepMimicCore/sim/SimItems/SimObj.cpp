...
void cSimObj::GetRotation(tVector &out_axis, double &out_theta) const
{
    ...
    out_theta = bt_q.getAngle(); // 获得轴角表示的角度大小
    out_axis[0] = axis[0];       // 获得轴角表示的轴朝向
    out_axis[1] = axis[1];
    out_axis[2] = axis[2];
    out_axis[3] = 0;
}
...
tQuaternion cSimObj::GetRotation() const
{
    ...
    btTransform trans =
        col_obj
            ->getWorldTransform(); // 局部->世界，因为getposition的时候，直接拿到原点就是位置。
    btQuaternion bt_q = trans.getRotation();
...
void cSimObj::SetRotation(const tQuaternion &q)
{
    btCollisionObject *col_obj = GetCollisionObject();
    btTransform trans = col_obj->getWorldTransform();
    btQuaternion bt_q = btQuaternion(
...
tMatrix3d cSimObj::GetLocalToWorldRotMat() const
{
    tMatrix local_to_world = GetWorldTransform();
    tMatrix3d mat = local_to_world.block(0, 0, 3, 3);
    return mat;
}

tVector cSimObj::CalcCOM() const { return GetPos(); }

tVector cSimObj::CalcCOMVel() const { return GetLinearVelocity(); }
...
void cSimObj::SetColGroup(short col_group) { mColGroup = col_group; }

short cSimObj::GetColMask() const { return mColMask; }

void cSimObj::SetColMask(short col_mask) { mColMask = col_mask; }

cSimObj::eType cSimObj::GetType() const { return mType; }

cShape::eShape cSimObj::GetShape() const { return cShape::eShapeNull; }
