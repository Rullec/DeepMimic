...
void cSimJoint::Init(std::shared_ptr<cWorldBase> &world,
                     const std::shared_ptr<cSimObj> &parent,
                     const std::shared_ptr<cSimObj> &child,
...
{
    return cMathUtil::TranslateMat(GetChildPos()) *
           cMathUtil::RotateMat(GetChildRot());
}

tMatrix cSimJoint::BuildJointParentTrans() const
{
    return cMathUtil::TranslateMat(GetParentPos()) *
           cMathUtil::RotateMat(GetParentRot());
}
...
tVector cSimJoint::GetAxisRel() const { return tVector(0, 0, 1, 0); }

bool cSimJoint::HasParent() const { return mParent != nullptr; }

bool cSimJoint::HasChild() const { return mChild != nullptr; }

int cSimJoint::GetID() const { return mParams.mID; }

void cSimJoint::SetID(int id) { mParams.mID = id; }

cKinTree::eJointType cSimJoint::GetType() const { return mParams.mType; }

tQuaternion cSimJoint::CalcWorldBaseRotation() const
{
...
const cSpAlg::tSpVec &cSimJoint::GetTau() const { return mTotalTau; }
...
void cSimJoint::ClearTau() { mTotalTau.setZero(); }

tVector cSimJoint::CalcWorldBasePos() const
{
    tVector world_pos;
    if (HasChild())
    {
...
tVector cSimJoint::CalcWorldBasePos(const tVector &local_pos) const
{
...
tVector cSimJoint::CalcWorldBaseVel() const
{
    if (HasChild())
    {
...
tVector cSimJoint::CalcWorldBaseVel(const tVector &local_pos) const
{
    if (HasChild())
    {
...
tVector cSimJoint::CalcWorldBaseAngVel() const
{
    if (HasChild())
    {
...
double cSimJoint::GetTorqueLimit() const { return mParams.mTorqueLimit; }

double cSimJoint::GetForceLimit() const { return mParams.mForceLimit; }

void cSimJoint::SetTorqueLimit(double lim) { mParams.mTorqueLimit = lim; }

void cSimJoint::SetForceLimit(double lim) { mParams.mForceLimit = lim; }

const tVector &cSimJoint::GetParentPos() const { return mParams.mParentPos; }

const tVector &cSimJoint::GetChildPos() const { return mParams.mChildPos; }
...
const tQuaternion &cSimJoint::GetChildRot() const { return mParams.mChildRot; }

const std::shared_ptr<cSimObj> &cSimJoint::GetParent() const { return mParent; }

const std::shared_ptr<cSimObj> &cSimJoint::GetChild() const { return mChild; }
...
const tVector &cSimJoint::GetLimLow() const { return mParams.mLimLow; }

const tVector &cSimJoint::GetLimHigh() const { return mParams.mLimHigh; }
...
double cSimJoint::GetPrismaticOffset() const
{
    ...
    if (HasMultBody())
    {
        // const btMultiBodySliderConstraint* prismatic =
        // reinterpret_cast<const
        // btMultiBodySliderConstraint*>(mMultiBodyCons.get()); delta =
        // prismatic->getPosition(0); delta /= scale;
        assert(false); // unsupported
    }
    else
    {
        const btSliderConstraint *prismatic =
            reinterpret_cast<const btSliderConstraint *>(mCons.get());
...
void cSimJoint::BuildConstraint(std::shared_ptr<cWorldBase> &world)
{
    switch (GetType())
    {
    case cKinTree::eJointTypeRevolute:
...
void cSimJoint::BuildConstraintRevolute(std::shared_ptr<cWorldBase> &world)
{
    ...
    {
        tVector euler0 =
            cMathUtil::QuaternionToEuler(GetParentRot(), eRotationOrder::XYZ);
        tVector euler1 =
            cMathUtil::QuaternionToEuler(GetChildRot(), eRotationOrder::XYZ);
        ...
        btVector3 anchor0 =
            scale * btVector3(static_cast<btScalar>(parent_pos[0]),
                              static_cast<btScalar>(parent_pos[1]),
                              static_cast<btScalar>(parent_pos[2]));
        anchor0_t.setOrigin(anchor0);
        anchor0_t.getBasis().setEulerZYX(static_cast<btScalar>(euler0[0]),
                                         static_cast<btScalar>(euler0[1]),
        ...
        btVector3 anchor1 =
            scale * btVector3(static_cast<btScalar>(child_pos[0]),
                              static_cast<btScalar>(child_pos[1]),
                              static_cast<btScalar>(child_pos[2]));
        anchor1_t.setOrigin(anchor1);
        anchor1_t.getBasis().setEulerZYX(static_cast<btScalar>(euler1[0]),
                                         static_cast<btScalar>(euler1[1]),
        ...
        cons =
            new btHingeConstraint(*(parent->GetSimBody()),
                                  *(child->GetSimBody()), anchor0_t, anchor1_t);

        // bullet limits are flipped
        cons->setLimit(-static_cast<btScalar>(mParams.mLimHigh[0]),
                       -static_cast<btScalar>(mParams.mLimLow[0]));
...
void cSimJoint::BuildConstraintPlanar(std::shared_ptr<cWorldBase> &world)
{
    assert(false); // unsupported
}

void cSimJoint::BuildConstraintPrismatic(std::shared_ptr<cWorldBase> &world)
{
    btScalar scale = static_cast<btScalar>(world->GetScale());
    ...
    {
        const tVector &euler0 =
            cMathUtil::QuaternionToEuler(GetParentRot(), eRotationOrder::XYZ);
        const tVector &euler1 =
            cMathUtil::QuaternionToEuler(GetChildRot(), eRotationOrder::XYZ);
        ...
        cSimRigidBody *parent = dynamic_cast<cSimRigidBody *>(mParent.get());
        cSimRigidBody *child = dynamic_cast<cSimRigidBody *>(mChild.get());
        cons = new btSliderConstraint(*(parent->GetSimBody()),
                                      *(child->GetSimBody()), anchor0_t,
                                      anchor1_t, true);

        cons->setLowerLinLimit(
            static_cast<btScalar>(scale * mParams.mLimLow[0]));
        cons->setUpperLinLimit(
            static_cast<btScalar>(scale * mParams.mLimHigh[0]));
...
void cSimJoint::BuildConstraintFixed(std::shared_ptr<cWorldBase> &world)
{
    btScalar scale = static_cast<btScalar>(world->GetScale());
    auto sim_link = dynamic_cast<const cSimBodyLink *>(mParent.get());
    if (sim_link == nullptr)
    {
        ...
        cons =
            new btFixedConstraint(*(parent->GetSimBody()),
                                  *(child->GetSimBody()), anchor0_t, anchor1_t);
        mCons = std::shared_ptr<btTypedConstraint>(cons);
    }
    else
...
void cSimJoint::BuildConstraintSpherical(std::shared_ptr<cWorldBase> &world)
{
    btScalar scale = static_cast<btScalar>(world->GetScale());
    auto parent_sim_link = dynamic_cast<const cSimBodyLink *>(mParent.get());
    auto child_sim_link = dynamic_cast<const cSimBodyLink *>(mChild.get());
    if (parent_sim_link == nullptr)
    {
        ...
        cons = new btGeneric6DofSpring2Constraint(*(parent->GetSimBody()),
                                                  *(child->GetSimBody()),
                                                  anchor0_t, anchor1_t);
    ...
    else if (child_sim_link == nullptr)
    {
        ...
        const tVector &parent_pos = GetParentPos();
        const tVector &child_pos = GetChildPos();
        auto cons = new btMultiBodyPoint2Point(
            parent_mult->GetMultBody().get(), parent_mult->GetJointID(),
            child_rb->GetSimBody().get(),
...
void cSimJoint::BuildPoseRevolute(Eigen::VectorXd &out_pose) const
{
    ...
    btHingeConstraint *hinge =
        reinterpret_cast<btHingeConstraint *>(mCons.get());
...
void cSimJoint::BuildPoseSpherical(Eigen::VectorXd &out_pose) const
{
    ...
    {
        btGeneric6DofSpring2Constraint *sphere =
            reinterpret_cast<btGeneric6DofSpring2Constraint *>(mCons.get());
        euler = tVector(-sphere->getAngle(0), -sphere->getAngle(1),
                        -sphere->getAngle(2), 0);
    }
...
{
    int param_size = GetParamSize();
    out_pose = std::numeric_limits<double>::quiet_NaN() *
               Eigen::VectorXd::Ones(param_size);
    assert(false); // unsupported
}
