...
cSimRigidBody::~cSimRigidBody() { RemoveFromWorld(); }
...
tVector cSimRigidBody::GetLinearVelocity(const tVector &local_pos) const
{
    ...
    btVector3 bt_vel = body->getVelocityInLocalPoint(
        static_cast<btScalar>(scale) *
        btVector3(static_cast<btScalar>(rel_pos[0]),
...
{
    auto &body = GetSimBody();
    body->setAngularVelocity(btVector3(static_cast<btScalar>(vel[0]),
                                       static_cast<btScalar>(vel[1]),
                                       static_cast<btScalar>(vel[2])));
}
...
void cSimRigidBody::SetDamping(double linear_damping, double angular_damping)
{
    ...
    body->setDamping(static_cast<btScalar>(linear_damping),
                     static_cast<btScalar>(angular_damping));
}

double cSimRigidBody::GetMass() const { return 1 / mSimBody->getInvMass(); }
...
void cSimRigidBody::ClearForces() { mSimBody->clearForces(); }
...
cFeaWorld::tConstraintHandle cSimRigidBody::GetConstraint(int c) const
{
    cFeaWorld::tConstraintHandle handle;
