...
{
    auto ok = std::dynamic_pointer_cast<cSimCharacter>(base);
    MIMIC_ASSERT(ok != nullptr);
    return ok;
}
...
cSimCharacter::~cSimCharacter() { RemoveFromWorld(); }
...
/*
        ...
        1. cCharcter::Init: Load Joints Skleton by calling cCharcter::Init.
   Read Json["Skeletons"]["Joints"] value and parse it. set up the final joint
   info matrix, which is so called "mJointMat"

        2. cCharcter::Init: Load Draw Shapes, in the second half part of
   cCharacter::Init. Prepared well to do rendering

        3. BuildSimBody: first load simulation info Json["BodyDefs"], then set
   up bullet simulation body etc including: succ &= BuildMultiBody(mMultiBody);
   // bullet multibody class succ &= BuildConstraints(mMultiBody);	//
   constraints connect between links
                        succ &= BuildBodyLinks();
   // build class SimBodyLink
                        succ &= BuildJoints();
   // build class SimBodyJoint
*/
bool cSimCharacter::Init(const std::shared_ptr<cWorldBase> &world,
                         const tParams &params)
{
    bool succ = true;
    // 在sim char后面还有一个Character
    bool succ_skeleton =
        cCharacter::Init(params.mCharFile, params.mLoadDrawShapes);
    succ &= succ_skeleton;
    SetID(params.mID);
    RemoveFromWorld();
    mWorld = world;
...
{
    // std::cout << "[warning] void cSimCharacter::PostUpdate(double time_step)
    // update link pos " << std::endl;
    UpdateLinkPos();
    UpdateLinkVel();
    BuildPose(mPose);
    BuildVel(mVel);
...
tQuaternion cSimCharacter::GetRootRotation() const
{
    ...
    // std::cout <<"error root rot.calcworldrot before = " <<
    // rot.coeffs().transpose() << std::endl; std::cout <<"error
    // mInvRootAttachRot = " << mInvRootAttachRot.coeffs().transpose() <<
    // std::endl;
    rot = mInvRootAttachRot * rot;
    if (mInvRootAttachRot.coeffs().segment(0, 3).norm() > 1e-5)
    {
        std::cout << "[error] cSimCharacter::GetRootRotation doesn't "
                     "support non-zero rest pose "
                  << mInvRootAttachRot.coeffs().transpose() << std::endl;
        exit(1);
    }
    // std::cout <<"error root rot.calcworldrot after = " <<
    // rot.coeffs().transpose() << std::endl;
    return rot;
}
...
void cSimCharacter::SetRootTransform(const tVector &pos, const tQuaternion &rot)
{
    ...
    // std::cout <<"pos = " << pos.transpose()<<" rot = " <<
    // rot.coeffs().transpose() << std::endl;
    tQuaternion root_rot =
        cKinTree::GetRootRot(mJointMat, mPose); // pose中的rotation
    ...
    // std::cout << "delta rot = " << delta_rot.coeffs().transpose() <<
    // std::endl;
...
void cSimCharacter::SetVel(const Eigen::VectorXd &vel)
{
    ...
    {
        mMultiBody->setBaseVel(
            world_scale * btVector3(root_vel[0], root_vel[1], root_vel[2]));
        mMultiBody->setBaseOmega(
            btVector3(root_ang_vel[0], root_ang_vel[1], root_ang_vel[2]));
    }
    else
    {
        ...
        tVector local_root_ang_vel =
            cMathUtil::QuatRotVec(world_to_root, root_ang_vel);
        Eigen::VectorXd root_params =
            Eigen::VectorXd::Zero(GetParamSize(root_id));
        ...
        default:
            MIMIC_ASSERT(false); // unsupported joint type
            break;
        }
...
tVector cSimCharacter::CalcJointPos(int joint_id) const
{
    ...
    else
    {
        ...
        MIMIC_ASSERT(parent_id != cKinTree::gInvalidJointID);
        MIMIC_ASSERT(IsValidBodyPart(parent_id));

        tVector attach_pt = cKinTree::GetAttachPt(mJointMat, joint_id);
        tVector part_attach_pt =
            cKinTree::GetBodyAttachPt(mBodyDefs, parent_id);
...
void cSimCharacter::CalcJointWorldRotation(int joint_id, tVector &out_axis,
...
tVector cSimCharacter::CalcCOMVel() const
{
    ...
    for (int i = 0; i < static_cast<int>(mBodyParts.size()); ++i)
    {
        if (IsValidBodyPart(i))
        {
            ...
            double mass = part->GetMass();
            tVector curr_vel =
                part->GetLinearVelocity(); // 充分证明这是质心处的线速度
...
std::shared_ptr<cSimBodyLink> cSimCharacter::GetBodyPart(int idx) const
{
    return mBodyParts[idx];
}
...
std::shared_ptr<cSimBodyLink> cSimCharacter::GetRootPart() const
{
...
bool cSimCharacter::HasVelExploded(double vel_threshold /*= 100.0*/) const
{
    ...
    for (int b = 0; b < num_bodies; ++b)
    {
        ...
        double max_val =
            std::max(vel.cwiseAbs().maxCoeff(), ang_vel.cwiseAbs().maxCoeff());
        if (max_val > vel_threshold)
        {
            std::cout << "invalid episode, body" << b
                      << ", velocity exploded = " << max_val << " > "
                      << vel_threshold << std::endl;
            return true;
        }
    }
    return false;
...
cSimCharacter::GetContactPts(int idx) const
{
    return GetBodyPart(idx)->GetContactPts();
}

const tEigenArr<cContactManager::tContactPt> &
cSimCharacter::GetContactPts() const
{
...
const void cSimCharacter::GetTotalContactPts(Eigen::VectorXd &out_contact) const
{
    ...
    std::shared_ptr<cWorldBase> cur_world = GetWorld();
    ...
    {
        std::cout << "[error] cSimCharacter::GetTotalContactPts: exceed "
                     "max contact size "
                  << contact_manager.GetNumTotalContactPts() * 7 << std::endl;
        exit(1);
    }
    ...
    for (int i = 0, cur_index = 0; i < contact_manager.GetNumEntries(); i++)
    {
        ...
        {
            int link_id = i;
            tVector3d contact_point_world = cur_pt.mPos.segment(0, 3),
            ...
            if (true == contact_point_world.hasNaN() ||
                true == contact_force_world.hasNaN())
            {
                std::cout << "[error] cSimCharacter::GetTotalContactPts: "
                             "Nan for link "
                          << i << ", pof = " << contact_point_world.transpose()
                          << ", force = " << cur_pt.mForce.transpose()
                          << std::endl;
                // exit(1);
            }
        }
...
{
    bool succ = true;
    // load important key "BodyDefs" , it is essential for simulation, including
    // mass, inertia, etc.
    ...
    {
        mInvRootAttachRot =
            cMathUtil::EulerToQuaternion(
                cKinTree::GetAttachTheta(mJointMat, GetRootID()),
                eRotationOrder::XYZ)
                .inverse();
...
bool cSimCharacter::BuildMultiBody(std::shared_ptr<cMultiBody> &out_body)
{
    ...
    mMultiBody = std::shared_ptr<cMultiBody>(new cMultiBody(
        num_joints, base_mass, base_intertia, fixed_base, false));
    ...
    for (int j = 0; j < num_joints; ++j)
    {
        ...
        cShape::eShape body_shape = cKinTree::GetBodyShape(
            mBodyDefs, j); // bodyshape就说明他是个球还是个胶囊还是个圆柱
        tVector body_size = cKinTree::GetBodySize(
            mBodyDefs, j); // bodysize就是json中的param1 - param2
        ...
        btCollisionShape *col_shape =
            BuildCollisionShape(body_shape, body_size);
        btVector3 inertia = btVector3(0, 0, 0);
        col_shape->calculateLocalInertia(static_cast<btScalar>(mass),
                                         inertia); // local inertia是本地轴向的
        // inertia: 没有经过scale

        // arg so many transforms...
        tQuaternion this_to_parent = cMathUtil::EulerToQuaternion(
            cKinTree::GetAttachTheta(mJointMat, j),
            eRotationOrder::XYZ); // 这个joint到parent的变换的欧拉角
        tQuaternion body_to_this = cMathUtil::EulerToQuaternion(
            cKinTree::GetBodyAttachTheta(mBodyDefs, j),
            eRotationOrder::XYZ); //　本地网格到joint 变换的欧拉角到四元数
        tQuaternion this_to_body =
            body_to_this.inverse(); // 从local joint到网格

        tQuaternion parent_body_to_parent = tQuaternion::Identity();
        if (parent_joint != gInvalidIdx)
        {
            parent_body_to_parent = cMathUtil::EulerToQuaternion(
                cKinTree::GetBodyAttachTheta(mBodyDefs, parent_joint),
                eRotationOrder::XYZ);
        }
        tQuaternion parent_to_parent_body = parent_body_to_parent.inverse();

        tQuaternion body_to_parent_body =
            parent_to_parent_body * this_to_parent * body_to_this;
        ...
        if (parent_joint != gInvalidIdx)
        {
            // 拿到parent body和本body的连接点,
            // 在body坐标系下，连接点的坐标 body's coordinate什么意思?
            parent_body_attach_pt =
                cKinTree::GetBodyAttachPt(mBodyDefs, parent_joint);
        }
        // 做了一个到parent到parent body的坐标系
        parent_body_attach_pt =
            cMathUtil::QuatRotVec(parent_to_parent_body, parent_body_attach_pt);

        tVector joint_attach_pt = cKinTree::GetAttachPt(
            mJointMat, j); // 铰链的连接点，在parent joint坐标系下
        tVector body_attach_pt =
            cKinTree::GetBodyAttachPt(mBodyDefs, j); // body也有连接点!
        joint_attach_pt =
            cMathUtil::QuatRotVec(parent_to_parent_body,
                                  joint_attach_pt); // 铰链的连接点也做了变换
        joint_attach_pt -=
            parent_body_attach_pt; //减去了parent body attach point是为什么?
                                   //运算说明在同一坐标系下..
        body_attach_pt =
            cMathUtil::QuatRotVec(body_to_this.inverse(), body_attach_pt);

        btTransform parent_body_to_body_trans = btTransform::getIdentity();
        parent_body_to_body_trans.setRotation(
            btQuaternion(parent_body_to_body.x(), parent_body_to_body.y(),
                         parent_body_to_body.z(), parent_body_to_body.w()));

        bool disable_parent_collision = true;

        // Eigen::Vector3d tmp_inertia = Eigen::Vector3d(inertia[0],
        // inertia[1], inertia[2]); std::cout<<"[init body] link " << j <<",
        // mass = " << mass <<", inertia = " << tmp_inertia.transpose() <<
        // std::endl;
        if (is_root && !fixed_base)
        {
            mMultiBody->setupFixed(
                j, static_cast<btScalar>(mass), inertia, parent_joint,
                parent_body_to_body_trans.getRotation(),
                world_scale * btVector3(joint_attach_pt[0], joint_attach_pt[1],
                                        joint_attach_pt[2]),
                world_scale * btVector3(body_attach_pt[0], body_attach_pt[1],
                                        body_attach_pt[2]),
        ...
        else
        {
            ...
            case cKinTree::eJointTypeRevolute:
            {
                ...
                mMultiBody->setupRevolute(
                    ...
                    world_scale * btVector3(joint_attach_pt[0],
                                            joint_attach_pt[1],
                                            joint_attach_pt[2]),
                    world_scale * btVector3(body_attach_pt[0],
                                            body_attach_pt[1],
                                            body_attach_pt[2]),
                    disable_parent_collision);
            }
        ...
        btMultiBodyLinkCollider *col_obj =
            new btMultiBodyLinkCollider(mMultiBody.get(), j);
    ...
    for (int j = 0; j < GetNumJoints(); ++j)
    {
            ...
            joint_type == cKinTree::eJointTypePrismatic)
        {
            ...
            if (lim_low[0] <= lim_high[1])
            {
                ...
                MIMIC_WARN("Adding joint constraints is not allowed because it "
                           "destroys the inverse dynamics of the solution for "
                           "joint {}",
                           GetBodyName(j));
                auto joint_cons =
                    std::shared_ptr<btMultiBodyJointLimitConstraint>(
                        new btMultiBodyJointLimitConstraint(
                            mMultiBody.get(), j, lim_low[0], lim_high[0]));
...
bool cSimCharacter::LoadBodyDefs(const std::string &char_file,
                                 Eigen::MatrixXd &out_body_defs)
{
    ...
    MIMIC_ASSERT(num_joints == num_body_defs);
    return succ;
}
...
btCollisionShape *cSimCharacter::BuildCollisionShape(const cShape::eShape shape,
...
bool cSimCharacter::BuildJoints()
{
    ...
    for (int j = 0; j < num_joints; ++j)
    {
        ...
        joint_params.mTorqueLimit =
            (is_root) ? 0 : cKinTree::GetTorqueLimit(mJointMat, j);
        joint_params.mForceLimit =
            (is_root) ? 0 : cKinTree::GetForceLimit(mJointMat, j);

        tVector child_attach_pt = cKinTree::GetBodyAttachPt(
            mBodyDefs, j); // 拿到BodyDefs里面的pt, 说的是网格
        tVector child_attach_theta = cKinTree::GetBodyAttachTheta(
            mBodyDefs, j); // BodyDefs里面的theta, 说的是网格,
                           // 这个角度说的是child joint
        tMatrix child_to_joint =
            cMathUtil::TranslateMat(child_attach_pt) *
            cMathUtil::RotateMat(child_attach_theta, eRotationOrder::XYZ);
        tMatrix joint_to_child = cMathUtil::InvRigidMat(
            child_to_joint); // 从joint 坐标系到link坐标系

        joint_params.mChildRot = cMathUtil::RotMatToQuaternion(
            joint_to_child); // 从joint 到link坐标系
        joint_params.mChildPos =
            cMathUtil::GetRigidTrans(joint_to_child); // joint在link坐标系位置
        ...
        {
            /*
                    JointMat中的角度t，是在parent系下
               joint坐标系原点的角度和位置
                    此时，如果要让joint坐标系下任何一个点P，变换到parent坐标系下面，就只需要Trans(t)
               * P了
            */
            tVector joint_attach_pt = cKinTree::GetAttachPt(mJointMat, j);
            tVector joint_attach_theta = cKinTree::GetAttachTheta(mJointMat, j);
            tVector parent_attach_pt =
                cKinTree::GetBodyAttachPt(mBodyDefs, parent_id);
            tVector parent_attach_theta =
                cKinTree::GetBodyAttachTheta(mBodyDefs, parent_id);

            tMatrix parent_to_parent_joint =
                cMathUtil::TranslateMat(parent_attach_pt) *
                cMathUtil::RotateMat(parent_attach_theta, eRotationOrder::XYZ);
            tMatrix joint_to_parent_joint =
                cMathUtil::TranslateMat(joint_attach_pt) *
                cMathUtil::RotateMat(joint_attach_theta, eRotationOrder::XYZ);
            tMatrix joint_to_parent =
                cMathUtil::InvRigidMat(parent_to_parent_joint) *
                joint_to_parent_joint;

            parent_link = GetBodyPart(parent_id);
            joint_params.mParentRot = cMathUtil::RotMatToQuaternion(
                joint_to_parent); // from joint coord to parent joint
                                  // coord, for a vec
            joint_params.mParentPos = cMathUtil::GetRigidTrans(
...
void cSimCharacter::BuildConsFactor(int joint_id, tVector &out_linear_factor,
                                    tVector &out_angular_factor) const
{
    cKinTree::eJointType joint_type =
        cKinTree::GetJointType(mJointMat, joint_id);
...
void cSimCharacter::BuildRootConsFactor(cKinTree::eJointType joint_type,
...
bool cSimCharacter::FixedBase() const
{
    int root_id = GetRootID();
    cKinTree::eJointType joint_type =
        cKinTree::GetJointType(mJointMat, root_id);
...
{
    MIMIC_ASSERT(idx >= 0 && idx < GetNumBodyParts());
...
void cSimCharacter::UpdateJoints()
{
    ...
    if (mEnableJointTorqueControl == false)
        std::cout
            << "[debug] cSimCharacter::UpdateJoints disable joint forces\n";
    int num_joints = GetNumJoints();
    for (int j = 1; j < num_joints; ++j)
    {
        cSimBodyJoint &joint = GetJoint(j);
...
void cSimCharacter::UpdateLinkVel()
{
    ...
    for (int b = 0; b < GetNumBodyParts(); ++b)
    {
        if (IsValidBodyPart(b))
        {
            ...
            tVector com_omega =
                tVector(bt_ang_vel[0], bt_ang_vel[1], bt_ang_vel[2], 0);
            com_vel /= world_scale;
            ...
            // std::cout << "[before] " << com_vel.transpose() <<
            // std::endl; std::cout << "[before] " <<
            // com_omega.transpose() << std::endl;
            ...
            // std::cout << "[after] " << com_vel.transpose() <<
            // std::endl; std::cout << "[after] " <<
            // com_omega.transpose() << std::endl;
...
short cSimCharacter::GetPartColMask(int part_id) const
{
    ...
    MIMIC_ASSERT(col_group < static_cast<int>(sizeof(short) * 8));
...
tVector cSimCharacter::GetPos() const { return GetRootPos(); }

void cSimCharacter::SetPos(const tVector &pos) { SetRootPos(pos); }
...
tQuaternion cSimCharacter::GetRotation() const { return GetRootRotation(); }
...
void cSimCharacter::SetRotation(const tQuaternion &q) { SetRootRotation(q); }
...
tVector cSimCharacter::GetLinearVelocity() const { return GetRootVel(); }
...
void cSimCharacter::SetLinearVelocity(const tVector &vel) { SetRootVel(vel); }

tVector cSimCharacter::GetAngularVelocity() const { return GetRootAngVel(); }
...
short cSimCharacter::GetColMask() const { return GetRootPart()->GetColMask(); }
...
void cSimCharacter::SetEnablejointTorqueControl(bool v_)
{
    ...
    // std::cout <<"[debug] cSimCharacter::SetEnablejointTorqueControl " << v_
    // << std::endl; exit(1);
}

std::string cSimCharacter::GetCharFilename() { return mCharFilename; }
...
cSimCharacter::GetConstraints() const
{
    return mCons;
}

void cSimCharacter::BuildJointPose(int joint_id,
                                   Eigen::VectorXd &out_pose) const
{
    ...
    if (is_root)
    {
        ...
        MIMIC_ASSERT(out_pose.size() == cKinTree::gRootDim);

        tVector root_pos = GetRootPos();
        tQuaternion root_rot = GetRootRotation();

        out_pose.segment(0, cKinTree::gPosDim) =
            root_pos.segment(0, cKinTree::gPosDim);
...
void cSimCharacter::BuildJointVel(int joint_id, Eigen::VectorXd &out_vel) const
{
    ...
    if (is_root)
    {
        ...
        MIMIC_ASSERT(out_vel.size() == cKinTree::gRootDim);

        tVector root_vel = GetRootVel();
        tVector ang_vel = GetRootAngVel();
        out_vel.segment(0, cKinTree::gPosDim) =
            root_vel.segment(0, cKinTree::gPosDim);
        out_vel.segment(cKinTree::gPosDim, cKinTree::gRotDim) =
            ang_vel.segment(0, cKinTree::gRotDim);
    }
    else
    {
        const cSimBodyJoint &joint = GetJoint(joint_id);
...
void cSimCharacter::BuildPose(Eigen::VectorXd &out_pose) const
{
    ...
    for (int j = 0; j < num_joints; ++j)
    {
        ...
        int param_offset = GetParamOffset(j);
        int param_size = GetParamSize(j);
        MIMIC_ASSERT(joint_pose.size() == param_size);
...
void cSimCharacter::BuildVel(Eigen::VectorXd &out_vel) const
{
    ...
    for (int j = 0; j < num_joints; ++j)
    {
        ...
        int param_offset = GetParamOffset(j);
        int param_size = GetParamSize(j);
        MIMIC_ASSERT(joint_vel.size() == param_size);
...
bool cSimCharacter::CheckFallContact() const
{
    ...
    for (int b = 0; b < num_parts; ++b)
    {
                    ...
                    // 如果他落在地上就算失败了。
        {
            ...
            // 脚不应该开，该落地的地方不应该开：脚的enablebody part
            // fall contact是0才对
            const auto &curr_part = GetBodyPart(b);
            bool has_contact =
                curr_part->IsInContact(); // 看这个link是否摔倒了,
                                          // 所以脚是不是应该...关了.
            if (has_contact)
            {
...
btCollisionObject *cSimCharacter::GetCollisionObject() { return nullptr; }
