...
void cSimCylinder::Init(const std::shared_ptr<cWorldBase> &world,
                        const tParams &params)
{
    mType = params.mType;
    btScalar mass = (params.mType == eTypeDynamic)
                        ? static_cast<btScalar>(params.mMass)
                        : 0;

    mColShape = std::unique_ptr<btCollisionShape>(
        world->BuildCylinderShape(params.mRadius, params.mHeight));

    btVector3 inertia(0, 0, 0);
    mColShape->calculateLocalInertia(mass, inertia);
    btRigidBody::btRigidBodyConstructionInfo cons_info(
        mass, this, mColShape.get(), inertia);
