...
cSimBodyJoint::~cSimBodyJoint() { Clear(); }

void cSimBodyJoint::Init(std::shared_ptr<cWorldBase> &world,
...
tVector cSimBodyJoint::GetAxisRel() const { return tVector(1, 0, 0, 0); }

bool cSimBodyJoint::HasParent() const { return mParent != nullptr; }

bool cSimBodyJoint::HasChild() const { return mChild != nullptr; }

cKinTree::eJointType cSimBodyJoint::GetType() const { return mType; }
...
bool cSimBodyJoint::IsRoot() const { return mParams.mID == 0; }
...
void cSimBodyJoint::AddTau(const Eigen::VectorXd &tau)
{
    ...
    case cKinTree::eJointTypeSpherical:
        ...
        mTotalTau[1] += tau[1];
        mTotalTau[2] += tau[2];
        // std::cout <<"total tau in joint = " << mTotalTau.transpose() <<
        // ", norm = " << mTotalTau.norm() << std::endl;
        break;
    default:
...
const cSpAlg::tSpVec &cSimBodyJoint::GetTau() const { return mTotalTau; }
...
void cSimBodyJoint::ClearTau() { mTotalTau.setZero(); }
...
double cSimBodyJoint::GetTorqueLimit() const { return mParams.mTorqueLimit; }

double cSimBodyJoint::GetForceLimit() const { return mParams.mForceLimit; }

void cSimBodyJoint::SetTorqueLimit(double lim) { mParams.mTorqueLimit = lim; }

void cSimBodyJoint::SetForceLimit(double lim) { mParams.mForceLimit = lim; }

tVector cSimBodyJoint::GetParentPos() const { return mParams.mParentPos; }

tVector cSimBodyJoint::GetChildPos() const { return mParams.mChildPos; }

tQuaternion cSimBodyJoint::GetChildRot() const { return mParams.mChildRot; }

tMatrix cSimBodyJoint::BuildJointChildTrans() const
{
    return cMathUtil::TranslateMat(mParams.mChildPos) *
           cMathUtil::RotateMat(mParams.mChildRot);
}

tMatrix cSimBodyJoint::BuildJointParentTrans() const
{
    return cMathUtil::TranslateMat(mParams.mParentPos) *
           cMathUtil::RotateMat(mParams.mParentRot);
}
...
const tVector &cSimBodyJoint::GetLimLow() const { return mParams.mLimLow; }

const tVector &cSimBodyJoint::GetLimHigh() const { return mParams.mLimHigh; }
...
void cSimBodyJoint::BuildPose(Eigen::VectorXd &out_pose) const
{
    ...
    case cKinTree::eJointTypePlanar:
    {
        ...
        tVector pos =
            axis2 * (data[2] / world_scale) + axis1 * (data[1] / world_scale);
...
void cSimBodyJoint::BuildVel(Eigen::VectorXd &out_vel) const
{
    ...
    case cKinTree::eJointTypePlanar:
    {
        ...
        tVector lin_vel =
            (data[2] / world_scale) * tVector(axis2[0], axis2[1], axis2[2], 0) +
...
void cSimBodyJoint::SetPose(const Eigen::VectorXd &pose)
{
    ...
    case cKinTree::eJointTypePlanar:
    {
        tVector pos = cMathUtil::QuatRotVec(mParams.mChildRot,
                                            tVector(pose[0], pose[1], 0, 0));
        const auto &link = mMultiBody->getLink(mParams.mID);

        data[2] = link.getAxisBottom(2).dot(btVector3(pos[0], pos[1], pos[2])) *
                  world_scale;
        data[1] = link.getAxisBottom(1).dot(btVector3(pos[0], pos[1], pos[2])) *
                  world_scale;
        data[0] = pose[2];
        break;
    }
...
void cSimBodyJoint::SetVel(const Eigen::VectorXd &vel)
{
    ...
    case cKinTree::eJointTypePlanar:
    {
        ...
        data[2] = link.getAxisBottom(2).dot(
                      btVector3(lin_vel[0], lin_vel[1], lin_vel[2])) *
                  world_scale;
        data[1] = link.getAxisBottom(1).dot(
                      btVector3(lin_vel[0], lin_vel[1], lin_vel[2])) *
                  world_scale;
        data[0] = ang_vel[2];
        break;
...
{
    btMultibodyLink::eFeatherstoneJointType bt_type =
        mMultiBody->getLink(mParams.mID).m_jointType;
...
void cSimBodyJoint::ApplyTauPlanar()
{
    ...
    btScalar bt_data[] = {
        static_cast<btScalar>(
            ...
            link.getAxisTop(0).dot(btVector3(torque[0], torque[1], torque[2]))),
        static_cast<btScalar>(world_scale * link.getAxisBottom(1).dot(btVector3(
                                                force[0], force[1], force[2]))),
        static_cast<btScalar>(world_scale *
                              link.getAxisBottom(2).dot(
...
void cSimBodyJoint::ApplyTauSpherical()
{
    ...
    double world_scale = mWorld->GetScale();
    torque = cMathUtil::QuatRotVec(mParams.mChildRot,
                                   torque); // from joint to link vec
    // std::cout <<", joint " << mParams.mID <<" apply torque = "  <<
    // torque.norm();
    btScalar bt_data[] = {
        static_cast<btScalar>(world_scale * world_scale * torque[0]),
        static_cast<btScalar>(world_scale * world_scale * torque[1]),
        static_cast<btScalar>(world_scale * world_scale * torque[2])};
    mMultiBody->addJointTorqueMultiDof(mParams.mID,
                                       bt_data); //输入的torque是world frame
}
