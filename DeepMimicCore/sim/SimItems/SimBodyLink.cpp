...
cSimBodyLink::~cSimBodyLink() { RemoveFromWorld(); }

void cSimBodyLink::Init(const std::shared_ptr<cWorldBase> &world,
                        const std::shared_ptr<cMultiBody> &mult_body,
                        const tParams &params)
{
    ...
    mColObj = std::unique_ptr<btMultiBodyLinkCollider>(
        mMultiBody->getLink(mJointID).m_collider);
...
tVector cSimBodyLink::GetLinearVelocity() const { return mLinVel; }
...
double cSimBodyLink::GetMass() const { return mMass; }

tVector cSimBodyLink::GetInertia() const { return mInertia; }

double cSimBodyLink::GetFriction() const { return mColObj->getFriction(); }
...
cShape::eShape cSimBodyLink::GetShape() const { return mObjShape; }
...
int cSimBodyLink::GetJointID() const { return mJointID; }
...
btCollisionObject *cSimBodyLink::GetCollisionObject() { return mColObj.get(); }
