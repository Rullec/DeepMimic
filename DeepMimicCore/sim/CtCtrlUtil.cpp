...
{
	std::cout <<"void cCtCtrlUtil::BuildOffsetScaleTorque(const Eigen::MatrixXd& joint_mat, int joint_id, Eigen::VectorXd& out_offset, Eigen::VectorXd& out_scale)" << std::endl;
	// 从joint_mat这个矩阵中，获取action的均值和标准差
	const double default_torque_lim = 300;
	const double default_force_lim = 3000;

	int joint_dim = cKinTree::GetParamSize(joint_mat, joint_id);	// joint有多少维度?
	cKinTree::eJointType joint_type = cKinTree::GetJointType(joint_mat, joint_id);	// joint是什么类别?
	out_offset = Eigen::VectorXd::Zero(joint_dim);	// mean为0
	out_scale = Eigen::VectorXd::Ones(joint_dim);	// scale为１

	// 获取torque的Lim
	double torque_lim = cKinTree::GetTorqueLimit(joint_mat, joint_id);	// torque的lim
	double force_lim = cKinTree::GetForceLimit(joint_mat, joint_id);	// torque的lim

	std::cout <<"joint " << joint_id <<" torque lim = "<< torque_lim << std::endl;
	std::cout <<"joint " << joint_id <<" force lim = "<< force_lim << std::endl;

	if (!std::isfinite(torque_lim))	//如果torque lim是无穷的话，就设置为缺省的300
	{
		torque_lim = default_torque_lim;
	}

	if (!std::isfinite(force_lim))	// 力也是这样，但是我并没有找到哪里有力的信息...
	{
		force_lim = default_force_lim;
	}
	std::cout <<"[scale torque lim check] joint " << joint_id <<" torque lim = " << torque_lim << std::endl;
	std::cout <<"[scale torque lim check] joint " << joint_id <<" force lim = " << force_lim << std::endl;
	...
	case cKinTree::eJointTypePlanar:
		out_scale.fill(1 / force_lim);
		out_scale[joint_dim - 1] = 1 / torque_lim;	// 对于那些具有滑动副的系统，我们才需要force lim
		break;
	case cKinTree::eJointTypeFixed:
		break;
	case cKinTree::eJointTypeSpherical:
		out_scale.fill(1 / torque_lim);
		out_scale[joint_dim - 1] = 0;	// 而对于球而言，out_scale 的最后一个数字是0，
		break;
	default:
		assert(false); // unsupported joint type
		break;
...
#if defined(ENABLE_PD_SPHERE_AXIS)
	tVector lim_low = cKinTree::GetJointLimLow(joint_mat, joint_id);	// low lim(torque), 3个方向上的low
	tVector lim_high = cKinTree::GetJointLimHigh(joint_mat, joint_id);	// hight lim(torque) high vector, 3个方向上的hight

	double val_low = lim_low.minCoeff();	// 挑３个low里面最小的
	double val_high = lim_high.maxCoeff();	// 3个high里面最大的
	bool valid_lim = val_high >= val_low;
	if (!valid_lim)	// 如果被判定为无效的话，就会设置成默认的val(也就是high < low，被称之为无效)
	{
		val_low = -gDefaultRotatePDBound;
		val_high = gDefaultRotatePDBound;
	}
