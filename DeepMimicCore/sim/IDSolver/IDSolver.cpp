...
void cIDSolver::SetGeneralizedVel(const tVectorXd &q)
{
    ...
    if (mFloatingBase == true)
    {
        ...
        // cBulletUtil::btVectorTotVector0(mMultibody->getBaseOmega());
        // tVector vel =
        // cBulletUtil::btVectorTotVector0(mMultibody->getBaseVel()); for
        // (int i = 0; i < 3; i++)q_dot(i) = omega[i]; for (int i = 3; i <
        // 6; i++)q_dot(i) = vel[i - 3];
        ...
        // tQuaternion world_to_base_rot =
        // cMathUtil::EulerAnglesToQuaternion(euler_angle_rot,
        // eRotationOrder::XYZ);
        // mMultibody->setBasePos(cBulletUtil::tVectorTobtVector(root_pos));
         ...
         link_id++)
    {
        ...
        case btMultibodyLink::eSpherical:
        {
            ...
            // // std::cout << 2<< std::endl;
            // tQuaternion rot_qua =
            // cMathUtil::EulerAnglesToQuaternion(joint_pos_euler,
            // eRotationOrder::ZYX);
            // // std::cout << 3<< std::endl;
            // btScalar bt_joint_pos[4] = {rot_qua.x(), rot_qua.y(),
            // rot_qua.z(), rot_qua.w()};
            // // std::cout << 4<< std::endl;
            // mMultibody->setJointPosMultiDof(link_id,
            // bt_joint_pos);
    ...
    // std::cout << "current after set q = " << q_dot_test.transpose() <<
    // std::endl; std::cout << "error = " << (q - q_dot_test).norm() <<
    // std::endl; std::cout <<"over cIDSolver::SetGeneralizedVel done\n";
    // exit(1);
}
...
void cIDSolver::RecordJointForces(std::vector<tVector> &mJointForces) const
{
    ...
    for (int i = 0; i < mMultibody->getNumLinks(); i++)
    {
        ...
        case btMultibodyLink::eRevolute:
        {
            ...
            mJointForces[i] = local_torque;
            // std::cout << "-[debug] cIDSolver::RecordJointForces
            // get joint "<< i << " raw force = " <<
            // mMultibody->getJointTorque(i) << std::endl; std::cout
            // << "-[debug] cIDSolver::RecordJointForces get joint
            // "<< i << " mul force = " << local_torque.transpose()
            // << std::endl; std::cout << "-[debug]
            // cIDSolver::RecordJointForces get joint "<< i << " axis
            // = " <<
            // cBulletUtil::btVectorTotVector0(mMultibody->getLink(i).getAxisTop(0)).transpose()
            // << std::endl;
            break;
        }
        case btMultibodyLink::eFixed:
    ...
    std::map<int, int> &mWorldId2InverseId) const
{
    ...
    for (int i = 0; i < num_contact_manifolds; i++)
    {
        const btPersistentManifold *manifold =
            mWorld->getDispatcher()->getInternalManifoldPointer()[i];
        ...
        for (int j = 0; j < num_contact_pts; j++)
        {
            ...
            tVector friction = linear_friction_force1 * lateral_friction_dir1 +
    ...
    const std::vector<tMatrix> &mLinkRot) const
{
    ...
    for (auto &cur_force : mContactForces)
    {
        ...
        // torque = " << contact_torque_local.transpose() << std::endl;
...
#ifdef DEBUG_STEP
    fout << "result = " << solved_joint_force_full_concated.transpose()
         << std::endl;
#endif
    ...
    const std::vector<tVector> &ext_torques) const
{
    ...
    for (int ID_link_id = 0; ID_link_id < mNumLinks; ID_link_id++)
    {
        ...
        // << std::endl; std::cout << "local to world rot multi = \n" <<
        // cBulletUtil::btMatrixTotMatrix1(mMultibody->getLinkCollider(ID_link_id
        // - 1)->getWorldTransform().getBasis()) << std::endl;; std::cout <<
        // "user force world = " << force_world.transpose() << std::endl;
        // std::cout << "user force local = " << force_local.transpose() <<
        // std::endl; std::cout << "user force torque world = " <<
        // force_torque_world.transpose() << std::endl; std::cout << "user
        // force torque local = " << force_torque_local.transpose() <<
        // std::endl; std::cout << "user torque world = " <<
        // torque_world.transpose() << std::endl; std::cout << "user torque
        // local = " << torque_local.transpose() << std::endl;
    }
}
    ...
    for (int i = 0; i < mNumLinks; i++)
    {
        if (0 == i)
        {
            ...
            if (true == mFloatingBase)
            {
                ...
                // std::cout <<"omega after = " <<
                // omega_after.transpose() << std::endl; std::cout
                // <<"quater before = " <<
                // quater_before.coeffs().transpose() << std::endl;
                // std::cout <<"quater after = " <<
                // quater_after.coeffs().transpose() << std::endl;
                // exit(1);
