...
{
	// 我想我知道建立结构的真正地方在哪儿了...
	// 这个函数就是用来建立body的，之前的只是在loadjson而已。
	bool succ = true;
...
bool cSimCharacter::BuildJoints()
{
	...
	/*
		坐标系中只有点，只有坐标。所谓的向量也是一个点，所有的变换都是对点的变换。
		如果想要变换一个起点不在原点的向量的话，那就把它当做是两个点的变换。

		旧坐标系为xyz，新坐标系为xyz'。从旧坐标系旋转t角度到达新坐标系。
		那么，对旧坐标系下的任一点p，在新坐标下的坐标是T(-t) * p
	*/
	for (int j = 0; j < num_joints; ++j)
	{
		bool is_root = cKinTree::IsRoot(mJointMat, j);
