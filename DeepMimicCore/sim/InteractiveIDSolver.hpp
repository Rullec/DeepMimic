...
protected:
    ...
    struct tSummaryTable{
        ...
        std::string mSampleTrajDir;     // the root dir for storaging sample trajectories
        std::string mIDTraindataDir;     // the root dir for storaging the results of Inverse Dynamic
        std::string mMrTrajDir;         // the root dir for storaging the results after motion retargeing
        ...
        struct tSingleEpochInfo{        // This struct records info about a single traj
            ...
            std::string mr_traj_filename;   // the filepath of traj filester after being motion retargeted
            std::string train_filename;    // the filepath of train data that generated by cOfflineSolver.
            // Attention: this key "train_filename" should always be set to null in cSampleSolver.
    ...
    std::string SaveTraj(tSaveInfo & mSaveInfo, const std::string & traj_dir, const std::string & traj_rootname) const;    // save trajectories
    ...
    void SaveTrainData(const std::string & dir, const std::string & filename, std::vector<tSingleFrameIDResult> &) const;      // save train data "*.train", only "state, action, reward " trio pair will be storaged in it.
