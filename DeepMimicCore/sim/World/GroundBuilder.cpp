...
bool cGroundBuilder::ParseParamsJson(const std::string &param_file,
                                     cGround::tParams &out_params)
{
    ...
    if (succ)
    {
        ...
        out_params.mGroundWidth =
            root.get(cGround::gGroundWidthKey, out_params.mGroundWidth)
                .asDouble();
        out_params.mVertSpacingX =
            root.get(cGround::gVertSpacingXKey, out_params.mVertSpacingX)
                .asDouble();
        out_params.mVertSpacingZ =
            root.get(cGround::gVertSpacingZKey, out_params.mVertSpacingZ)
                .asDouble();
        ...
        if (!root[cGround::gParamsKey].isNull())
        {
            ...
            if (num > 0)
            {
                for (int i = 0; i < num; ++i)
                {
                    ...
                    succ &= ParseParamsJason(out_params.mType,
                                             params_arr.get(i, 0), curr_params);
...
void cGroundBuilder::BuildGround(const std::shared_ptr<cWorldBase> &world,
...
bool cGroundBuilder::ParseParamsJason(cGround::eType type,
...
void cGroundBuilder::BuildGroundPlane(const std::shared_ptr<cWorldBase> &world,
                                      const cGround::tParams &params,
                                      std::shared_ptr<cGround> &out_ground)
{
    std::shared_ptr<cGroundPlane> ground_plane =
        std::shared_ptr<cGroundPlane>(new cGroundPlane());
