...
#include "sim/SimItems/SimRigidBody.h"
...
public:
    ...
    virtual void Update(double time_elapsed, const tVector &bound_min,
                        const tVector &bound_max);
    virtual void Clear();

    virtual double SampleHeight(const tVector &pos) const;
    virtual double SampleHeight(const tVector &pos,
                                bool &out_valid_sample) const;
    virtual void SampleHeight(const Eigen::MatrixXd &pos,
                              Eigen::VectorXd &out_h) const;
    virtual void SampleHeightVel(const tVector &pos, double &out_h,
                                 tVector &out_vel,
                                 bool &out_valid_sample) const;
    ...
    virtual void SamplePlacement(const tVector &origin, tVector &out_pos,
                                 tQuaternion &out_rot);
...
protected:
    ...
    virtual void CalcBlendParams(double blend,
