...
public:
    ...
    // each bodypart and rigid body has its own contact handle, use to storage
    // the id & flags
    ...
    struct tContactPt
    {
        ...
        bool mIsSelfCollision; // self collision flag, only works in Multibody
                               // case.
...
protected:
    ...
    cWorldBase *mWorld;
    tEigenArr<tContactEntry>
        mContactEntries; // each links has the same entry, collect all these
                         // entries together
