...
void cContactManager::Update()
{
    ...
    for (int i = 0; i < num_manifolds; ++i)
    {
        ...
        for (int j = 0; j < num_contacts; ++j)
        {
            ...
            // std::cout << "later print" << std::endl;
            // bt2eigen(pt.m_lateralFrictionDir1); std::cout << "force 1
            // = " << linear_friction_force1 << ", direction = " <<
            // lateral_friction_dir1.transpose() << std::endl; std::cout
            // << "force 2 = " << linear_friction_force2 << ", direction
            // = " << lateral_friction_dir2.transpose() << std::endl;
            ...
            // if (pt.getDistance() <= dist_tol)
            {
                ...
                if (valid_contact)
                {
                    ...
                    // 看摩擦力: 没有用，只是一些系数
                    // std::cout << "------------" << std::endl;
                    // std::cout << pt.m_combinedFriction <<
                    // std::endl;; std::cout <<
                    // pt.m_combinedRollingFriction << std::endl;;
                    ////std::cout << pt.m_lateralFrictionDir1[0] <<
                    /// std::endl;; /std::cout <<
                    /// pt.m_lateralFrictionDir2 << std::endl;;
                    // b3printf(pt.m_lateralFrictionDir1);
                    // b3printf(pt.m_lateralFrictionDir2);

                    // std::cout << pt.m_combinedSpinningFriction <<
                    // std::endl;; std::cout << pt.m_frictionCFM <<
                    // std::endl;;
                }
            }
        }
