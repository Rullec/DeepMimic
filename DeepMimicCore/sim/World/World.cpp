...
void cFeaWorld::Init(const tParams &params)
{
    ...
    mCollisionConfig = std::unique_ptr<btDefaultCollisionConfiguration>(
        new btDefaultCollisionConfiguration());
    mCollisionDispatcher = std::unique_ptr<btCollisionDispatcher>(
        new btCollisionDispatcher(mCollisionConfig.get()));
...
void cFeaWorld::Update(double time_elapsed)
{
    ...
    btScalar timestep = static_cast<btScalar>(time_elapsed);
    MIMIC_ASSERT(mSimWorld != nullptr);
                                ...
                                int col_filter_group, int col_filter_mask)
{
    ...
    // mSimWorld->getNumMultibodies() << " " << " group = " << col_filter_group
    // << ", mask = " << col_filter_mask << std::endl; std::cout <<"[add
    // collision] add a object as " << col_filter_group <<" " << col_filter_mask
    // <<std::endl;
...
void cFeaWorld::AddCharacter(cSimCharacter &sim_char)
{
...
void cFeaWorld::RemoveCharacter(cSimCharacter &sim_char)
{
    const auto &constraints = sim_char.GetConstraints();
    for (int c = 0; c < static_cast<int>(constraints.size()); ++c)
    {
...
double cFeaWorld::GetDefaultLinearDamping() const { return mDefaultLinearDamping; }
...
btCylinderShape *cFeaWorld::BuildCylinderShape(double radius, double height) const
{
    btScalar scale = static_cast<btScalar>(GetScale());
...
std::shared_ptr<btDynamicsWorld> cFeaWorld::GetInternalWorld()
{
