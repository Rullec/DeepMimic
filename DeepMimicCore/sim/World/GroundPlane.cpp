...
bool cGroundPlane::ParseParamsJson(const Json::Value &json,
...
void cGroundPlane::Init(const std::shared_ptr<cWorldBase> &world,
                        const tParams &params)
{
    ...
    mColShape =
        std::unique_ptr<btCollisionShape>(new btStaticPlaneShape(normal, w));

    btRigidBody::btRigidBodyConstructionInfo cons_info(0, this, mColShape.get(),
                                                       btVector3(0, 0, 0));
...
void cGroundPlane::Update(double time_elapsed, const tVector &bound_min,
...
double cGroundPlane::SampleHeight(const tVector &pos,
                                  bool &out_valid_sample) const
{
    const btStaticPlaneShape *shape =
        reinterpret_cast<btStaticPlaneShape *>(mColShape.get());
...
void cGroundPlane::SampleHeight(const Eigen::MatrixXd &pos,
                                Eigen::VectorXd &out_h) const
{
