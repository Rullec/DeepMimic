...
void cGround::Init(const std::shared_ptr<cWorldBase> &world, const tParams &params)
{
    mParams = params;
    SetupRandGen();

    cSimRigidBody::Init(world);
    UpdateContact(cWorldBase::eContactFlagEnvironment, cWorldBase::eContactFlagAll);
...
void cGround::Update(double time_elapsed, const tVector &bound_min,
...
void cGround::SampleHeight(const Eigen::MatrixXd &pos,
...
void cGround::SampleHeightVel(const tVector &pos, double &out_h,
                              tVector &out_vel, bool &out_valid_sample) const
{
...
void cGround::SamplePlacement(const tVector &origin, tVector &out_pos,
                              tQuaternion &out_rot)
{
    out_pos.setZero();
