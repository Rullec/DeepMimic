...
cController::cController() { mChar = nullptr; }

cController::~cController() {}

void cController::Init(cSimCharacterBase *character)
{
...
void cController::Reset() { SetMode(eModeActive); }
...
bool cController::IsValid() const { return mValid; }
...
int cController::GetNumOptParams() const { return 0; }
...
void cController::SetOptParams(const Eigen::VectorXd &params,
                               Eigen::VectorXd &out_params) const
{
    out_params = params;
}
...
#include <iostream>
void cController::OutputOptParams(const std::string &file,
                                  const Eigen::VectorXd &params) const
{
    ...
    {
        std::cout << "[error] cController::OutputOptParams: open file failed:"
                  << file << std::endl;
        exit(1);
    }
}
...
bool cController::IsActive() const { return mMode != eModeInactive; }

void cController::SetMode(eMode mode) { mMode = mode; }

const cSimCharacterBase *cController::GetChar() const { return mChar; }

std::string cController::GetControllerFile() const { return mControllerFile; }

bool cController::ParseParams(const Json::Value &json) { return true; }