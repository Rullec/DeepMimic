...
const std::string gPDParamKeys[cPDController::eParamMax] = {
    ...
    "TargetTheta5", "TargetTheta6", "TargetVel0",   "TargetVel1",
    "TargetVel2",   "TargetVel3",   "TargetVel4",   "TargetVel5",
    "TargetVel6",   "UseWorldCoord"};

bool cPDController::LoadParams(const std::string &file,
                               Eigen::MatrixXd &out_buffer)
{
...
bool cPDController::LoadParams(const Json::Value &root,
                               Eigen::MatrixXd &out_buffer)
{
    bool succ = true;
...
cPDController::cPDController() : cController() { Clear(); }

cPDController::~cPDController() {}

void cPDController::Init(cSimCharacterBase *character, const tParams &params)
{
...
void cPDController::UpdateControlForce(double time_step,
...
void cPDController::SetKp(double kp) { mParams[eParamKp] = kp; }

double cPDController::GetKp() const { return mParams[eParamKp]; }
...
void cPDController::SetKd(double kd) { mParams[eParamKd] = kd; }
...
void cPDController::GetTargetTheta(Eigen::VectorXd &out_theta) const
{
    ...
    */
    ...
    if (UseWorldCoord() == true && joint_type == cKinTree::eJointTypeSpherical)
    {
        ...
        tar_q =
            rel_rot * diff; // Add this difference to current relative rotation
...
void cPDController::CalcJointTauPlanar(double time_step,
                                       Eigen::VectorXd &out_joint_tau)
{
...
void cPDController::CalcJointTauPrismatic(double time_step,
                                          Eigen::VectorXd &out_joint_tau)
{
    const cSimBodyJoint &joint = GetJoint();
...
void cPDController::CalcJointTauFixed(double time_step,
                                      Eigen::VectorXd &out_joint_tau)
{
    int dim = GetJointDim();
    out_joint_tau = Eigen::VectorXd::Zero(dim);
...
void cPDController::CalcJointTauSpherical(double time_step,
                                          Eigen::VectorXd &out_joint_tau)
{
    int dim = GetJointDim();
    out_joint_tau = Eigen::VectorXd::Zero(dim);
