...
std::string cCtVelController::GetName() const { return "ct_vel"; }
    ...
    for (int j = 0; j < num_joints; ++j)
    {
        if (mPDCtrl.IsValidPDCtrl(j))
        {
            ...
            Eigen::VectorXd theta_dot =
                mAction.segment(param_offset, param_size);
            mPDCtrl.SetTargetVel(j, theta_dot);
        }
    }
}

void cCtVelController::BuildJointActionBounds(int joint_id,
...
void cCtVelController::BuildJointActionOffsetScale(
    int joint_id, Eigen::VectorXd &out_offset, Eigen::VectorXd &out_scale) const
{
    const Eigen::MatrixXd &joint_mat = mChar->GetJointMat();
    cCtCtrlUtil::BuildOffsetScaleVel(joint_mat, joint_id, out_offset,
                                     out_scale);
}