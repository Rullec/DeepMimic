...
void cCtController::SetUpdateRate(double rate) { mUpdateRate = rate; }

double cCtController::GetUpdateRate() const { return mUpdateRate; }
...
void cCtController::BuildStateOffsetScale(Eigen::VectorXd &out_offset,
...
void cCtController::BuildActionBounds(Eigen::VectorXd &out_min,
...
double cCtController::GetCyclePeriod() const { return mCyclePeriod; }

void cCtController::SetCyclePeriod(double period) { mCyclePeriod = period; }
...
bool cCtController::ParseParams(const Json::Value &json)
{
    ...
    mEnablePhaseInput =
        json.get("EnablePhaseInput", mEnablePhaseInput).asBool();
    mEnablePhaseAction =
        json.get("EnablePhaseAction", mEnablePhaseAction).asBool();
    mRecordWorldRootPos =
...
void cCtController::UpdateBuildTau(double time_step, Eigen::VectorXd &out_tau)
{
    ...
    for (int j = root_id + 1; j < num_joints; ++j)
    {
        ...
        out_tau.segment(param_offset, param_size) =
            mAction.segment(retarget_offset, retarget_size);
    }
}
...
void cCtController::BuildActionOffsetScale(Eigen::VectorXd &out_offset,
                                           Eigen::VectorXd &out_scale) const
{
    // 在controller中，设置action offset和scale,
    // 也就是一个action该服从的高斯分布的mean 和std 起码对于初始而言，是这样的。
    ...
    int ctrl_offset =
        GetActionCtrlOffset(); // 获取action ctrl offset? 这个不是1就是0,
                               // 表明打开阶段性action.
    // 由于enableXX必然是false,所以ctrl_offset必然是0
    // std::cout <<"void cCtController::BuildActionOffsetScale(Eigen::VectorXd&
    // out_offset, Eigen::VectorXd& out_scale) const, out_Scale1[0] = "  <<
    // out_scale[0]<<std::endl;
    if (mEnablePhaseAction)
    {
    ...
    // std::cout <<"void cCtController::BuildActionOffsetScale(Eigen::VectorXd&
    // out_offset, Eigen::VectorXd& out_scale) const, out_Scale2[0] = "  <<
    // out_scale[0]<<std::endl;
    for (int j = root_id + 1; j < num_joints; ++j)
    {
        ...
        {
            int param_offset =
                mChar->GetParamOffset(j); // 对于这个joint而言，他从哪里开始?
            int param_size = mChar->GetParamSize(j); // 这个joint占据几个自由度?
            // std::cout <<"[get scale and offset] joint " << j <<",
            // param_offset = " << param_offset <<", size = " <<
            // param_size << std::endl;
            ...
            if (param_size > 0)
            {
                ...
                if (curr_scale.allFinite() == false)
                {
                    std::cout << "[error] "
                                 "cCtController::"
                                 "BuildActionOffsetScale joint "
                              << j
                              << " scale illegal = " << curr_scale.transpose()
                              << std::endl;
                    exit(1);
                }
                ...
                // {
                // 	std::cout <<"[get scale and offset] joint "<< j
                // <<" set 0th position = " << out_offset[0] <<
                // std::endl; 	std::cout <<"[get scale and
                // offset] its cur_scale = " <<
                // curr_scale.transpose() << std::endl; std::cout
                // <<"[get scale and offset] its cur_offset = " <<
                // curr_offset.transpose() << std::endl;
                // }
            }
        }
    }
    // std::cout <<"void cCtController::BuildActionOffsetScale(Eigen::VectorXd&
    // out_offset, Eigen::VectorXd& out_scale) const, out_Scale3[0] = "  <<
    // out_scale[0]<<std::endl;
    // abort();
}
...
void cCtController::BuildStateNormGroups(Eigen::VectorXi &out_groups) const
{
    ...
    if (mEnablePhaseInput)
    {
        ...
        int phase_size = GetStatePhaseSize();
        out_groups.segment(phase_offset, phase_size) =
            phase_group * Eigen::VectorXi::Ones(phase_size);
    }
}
...
void cCtController::RecordState(Eigen::VectorXd &out_state)
{
    ...
    // cCtController::RecordState(Eigen::VectorXd& out_state) " << std::endl;
...
std::string cCtController::GetName() const { return "ct"; }
...
int cCtController::GetStatePoseSize() const
{
    ...
    int size =
        mChar->GetNumBodyParts() * (pos_dim + rot_dim) + 1; // +1 for root y
...
int cCtController::GetStatePhaseOffset() const { return 0; }

int cCtController::GetActionPhaseOffset() const { return 0; }
...
int cCtController::GetActionCtrlOffset() const { return GetActionPhaseSize(); }
...
int cCtController::GetActionCtrlSize() const
{
    ...
    // underactuated system, root dof cannot be controlled
    ctrl_size -= root_size;
    return ctrl_size;
}

void cCtController::BuildStatePhaseOffsetScale(
    Eigen::VectorXd &phase_offset, Eigen::VectorXd &phase_scale) const
{
    double offset = -0.5;
    double scale = 2;
    int phase_size = GetStatePhaseSize();
...
{
    /*
            This function is used to build the 2nd part of record state:
       state_pose. (record_state = [phase, state_pose, state_vel])
    ...
    */
    ...
    root_pos_rel =
        origin_trans *
        root_pos_rel; // 把Y轴上的旋转和XZ轴上的位移，变换到root坐标系中。
    //	直观一点说，就是root pos的Y轴旋转归0(变到root坐标系下), root
    // pos的XZ坐标归0。

    root_pos_rel[3] = 0; // 这个变换，相当于消除了
    // std::cout << "root_pos_rel = " << root_pos_rel.transpose() << std::endl;
    // std::cout <<"root_pos_rel(in root local coordinate) = " <<
    // root_pos_rel.transpose() << std::endl;

    out_pose = Eigen::VectorXd::Zero(GetStatePoseSize());
    out_pose[0] =
        root_pos_rel[1]; // out_pose(不是record_state)的第一个数字，是root
                         // pos的Y轴坐标
    ...
    tQuaternion mirror_inv_origin_quat =
        origin_quat
            .conjugate(); // 共轭，反向旋转。root到世界换系旋转。把Y轴上的旋转和XZ轴上的位移，从root变换到世界坐标系中。
    mirror_inv_origin_quat = cMathUtil::MirrorQuaternion(
        mirror_inv_origin_quat, cMathUtil::eAxisZ); // 只把Z轴给镜像掉
    ...
    for (int i = 0; i < num_parts; ++i)
    {
        ...
        if (mChar->IsValidBodyPart(part_id))
        {
            ...
            // std::cout << "cCtController::BuildStatePose, get pos from
            // bt = " << curr_pos.transpose()<<std::endl;
            if (mRecordWorldRootPos && i == root_id)
            {
                ...
                if (flip_stance)
                {
                    ...
                    curr_pos =
                        cMathUtil::QuatRotVec(mirror_inv_origin_quat, curr_pos);
                }
            }
            else
            {
            ...
            // std::cout << "cCtController::BuildStatePose, pose after
            // trans = " << curr_pos.transpose()<<std::endl;

            // 到这行为止: curr_pos是root坐标系下，link的位置,
            // 放到pose里面去. 3*1
            out_pose.segment(idx, pos_dim) = curr_pos.segment(0, pos_dim);
            idx += pos_dim; // 索引加3

            tQuaternion curr_quat =
                curr_part->GetRotation(); // 获取rotation(link的)
            if (mRecordWorldRootRot && i == root_id)
            {
                if (flip_stance)
                {
                    curr_quat = origin_quat * curr_quat;
                    curr_quat = cMathUtil::MirrorQuaternion(curr_quat,
                                                            cMathUtil::eAxisZ);
            ...
            else
            {
                ...
                if (flip_stance)
                {
                    curr_quat = cMathUtil::MirrorQuaternion(
                        curr_quat,
                        cMathUtil::eAxisZ); // 为什么要翻转Z轴
                }
            }
            ...
            // std::cout <<"******************" << std::endl;
            out_pose.segment(idx, rot_dim) =
                cMathUtil::QuatToVec(curr_quat).segment(0, rot_dim);
...
void cCtController::BuildStateVel(Eigen::VectorXd &out_vel) const
{
    ...
    mirror_inv_origin_quat =
        cMathUtil::MirrorQuaternion(mirror_inv_origin_quat, cMathUtil::eAxisZ);
    ...
    for (int i = 0; i < num_parts; ++i)
    {
        ...
        if (mRecordWorldRootRot && i == root_id)
        {
            if (flip_stance)
            {
                ...
                curr_vel =
                    cMathUtil::QuatRotVec(mirror_inv_origin_quat, curr_vel);
            }
        }
        else
        {
        ...
        if (mRecordWorldRootRot && i == root_id)
        {
            if (flip_stance)
            {
                ...
                curr_ang_vel =
                    cMathUtil::QuatRotVec(mirror_inv_origin_quat, curr_ang_vel);
            }
        }
        else
        {
        ...
        out_vel.segment(idx, rot_dim - 1) =
            curr_ang_vel.segment(0, rot_dim - 1);
        idx += rot_dim - 1;
    }
}
...
void cCtController::BuildJointActionBounds(int joint_id,
...
void cCtController::BuildJointActionOffsetScale(
    int joint_id, Eigen::VectorXd &out_offset, Eigen::VectorXd &out_scale) const
{
    const Eigen::MatrixXd &joint_mat = mChar->GetJointMat();
    cCtCtrlUtil::BuildOffsetScaleTorque(joint_mat, joint_id, out_offset,
                                        out_scale);
}

bool cCtController::FlipStance() const { return false; }

int cCtController::RetargetJointID(int joint_id) const { return joint_id; }
