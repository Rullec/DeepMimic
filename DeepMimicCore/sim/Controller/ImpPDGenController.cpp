...
*/
tMatrixXd cPDCtrl::CalcDCtrlForceDx_Approx(double dt)
{
    // 1. form the matrix dq/dx, dqdot/dx, calculate some prerequisite
    auto gen_char = dynamic_cast<cSimCharacterGen *>(mChar);
    int dof = gen_char->GetNumOfFreedom();
    int x_size = dof * 2;
    tMatrixXd dq_dx = tMatrixXd::Zero(dof, x_size),
              dqdot_dx = tMatrixXd::Zero(dof, x_size);
    dq_dx.block(0, 0, dof, dof).setIdentity();
    dqdot_dx.block(0, dof, dof, dof).setIdentity();
    const tMatrixXd &Kp = this->mKp.asDiagonal().toDenseMatrix(),
                    &Kd = this->mKd.asDiagonal().toDenseMatrix();

    const tMatrixXd &M = gen_char->GetMassMatrix(),
                    &C = gen_char->GetCoriolisMatrix();

    /*
        2. calculate d(qddot_pred)/dx
        - (M + dt * Kd).inv() 
    /    * 
        (Kp * dqdx + (dt * Kp + Kd + C) * dqdotdx)
    */
    const tMatrixXd &dqddotpred_dx =
        (M + dt * Kd).inverse() * (Kp * dq_dx + (dt * Kp + Kd + C) * dqdot_dx);

    /*
        3. calculate the final dudx
        dudx = 
            -Kp *(dqdx + dt * dqdotdx) 
            - Kd * (dqdotdx + dt * dqddotpred_dx)
    */
    const tMatrixXd &dudx =
        -Kp * (dq_dx + dt * dqdot_dx) - Kd * (dqdot_dx + dt * dqddotpred_dx);

    MIMIC_ASSERT(dudx.hasNaN() == false);
