...
void cImpPDController::Init(cSimCharacterBase *character,
                            const Eigen::MatrixXd &pd_params,
                            const tVector &gravity)
{
    cSimCharacter *sim_char = dynamic_cast<cSimCharacter *>(character);
    MIMIC_ASSERT(sim_char != nullptr);
    std::shared_ptr<cRBDModel> model = BuildRBDModel(*sim_char, gravity);
...
void cImpPDController::UpdateControlForce(double time_step,
...
void cImpPDController::InitGains()
{
    ...
    for (int j = 0; j < GetNumJoints(); ++j)
    {
        ...
        if (pd_ctrl.IsValid())
        {
            ...
            mKp.segment(param_offset, param_size) =
                Eigen::VectorXd::Ones(param_size) * kp;
            mKd.segment(param_offset, param_size) =
                Eigen::VectorXd::Ones(param_size) * kd;
        }
    }
}

std::shared_ptr<cRBDModel>
cImpPDController::BuildRBDModel(const cSimCharacter &character,
                                const tVector &gravity) const
{
    std::shared_ptr<cRBDModel> model =
        std::shared_ptr<cRBDModel>(new cRBDModel());
...
void cImpPDController::CalcControlForces(double time_step,
                                         Eigen::VectorXd &out_tau)
{
    ...
    const Eigen::VectorXd &pose =
        mChar->GetPose(); // get current character's pose, quaternions for each
                          // joint
    const Eigen::VectorXd &vel =
        mChar->GetVel(); // get char's vel: vel = d(pose)/ dt, the differential
    ...
    {
        fout << "----------------------------\n";
        fout << "pose = " << pose.transpose() << " | size = " << pose.size()
             << std::endl;
        fout << "vel = " << vel.transpose() << " | size = " << vel.size()
             << std::endl;
        fout << "tar pose = " << tar_pose.transpose()
             << " | size = " << tar_pose.size() << std::endl;
        fout << "tar vel = " << tar_vel.transpose()
             << " | size = " << tar_vel.size() << std::endl;
...

    {
        ...
        // pose_inc = dqdt = q' = 0.5 * w * q, differential of quaternions
        // (pose)
        Eigen::VectorXd pose_inc;
        const Eigen::MatrixXd &joint_mat = mChar->GetJointMat();
        // use pose & vel to calculate "pose_inc"
        cKinTree::VelToPoseDiff(joint_mat, pose, vel,
                                pose_inc); // pose_inc = dqdt
        ...
        pose_inc =
            pose +
            timestep *
                pose_inc; // pose_cur + timestep * dqdt = pose_next (predicted)
        // std::cout <<"cImpPDController::CalcControlForces pose next inc = " <<
        // pose_inc.transpose() << std::endl;
        cKinTree::PostProcessPose(
            joint_mat, pose_inc); // normalize, quaternions make sense
        ...
        if (true == mEnableSolvePDTargetTest)
        {
            ...
            this->SolvePDTargetByTorque(timestep, pose, vel, out_tau,
                                        solved_pd_target);
            // std::cout << "truth pose = " << tar_pose.transpose() <<
            // std::endl; std::cout << "solved pose = " <<
            // wait_pd_target.transpose() << std::endl;
            tVectorXd diff = (tar_pose - solved_pd_target);
            for (int i = 0; i < diff.size(); i++)
            {
                if (std::abs(diff[i]) < 1e-10 ||
                    std::abs((tar_pose[i] + solved_pd_target[i])) < 1e-7)
                    diff[i] = 0;
            }
            if (diff.segment(7, diff.size() - 7).norm() > 1e-8)
            {
                std::cout << "[warning] cImpPDController::CalcControlForces: "
                             "truth target pose = "
                          << tar_pose.transpose() << std::endl;
                std::cout << "[warning] cImpPDController::CalcControlForces: "
                             "solve target pose = "
                          << solved_pd_target.transpose() << std::endl;
                std::cout << "\n[warning] "
                             "cImpPDController::CalcControlForces: "
                             "solve pd target diff = "
                          << diff.transpose() << std::endl;
                // exit(1);
            }
            // std::cout <<"[log] cImpPDController solve PD Target
            // accurately\n"; std::cout <<"[log] cImpPDController solved PD
            // target = " << solved_pd_target.transpose() << std::endl;
            // std::cout <<"[log] cImpPDController truth PD target = " <<
            // solved_pd_target.transpose() << std::endl;
...
{
    MIMIC_DEBUG("SetEnableSolvePDTargetTest = {}", enable);
 ...
 */
void cImpPDController::SolvePDTargetByTorque(double timestep,
                                             ...
                                             tVectorXd &PDTarget)
{
    ...
    mPDTargetSolver->SetParams(timestep, mRBDModel->GetMassMat(),
                               mRBDModel->GetBiasForce(), mKp_v, mKd_v);
