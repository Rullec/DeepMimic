...
{
public:
    cReverseController(cSimCharacterBase *sim_char);
    void CalcPDTarget(const tVectorXd &input_torque,
                      const tVectorXd &input_cur_pose,
                      const tVectorXd &input_cur_vel,
                      tVectorXd &output_pd_target);
    void CalcAction(const tVectorXd &input_torque,
                    const tVector &input_cur_pose,
                    const tVectorXd &input_cur_vel, tVector &output_action);
    void SetParams(double timestep, const Eigen::MatrixXd &M,
                   const Eigen::MatrixXd &C, const Eigen::VectorXd &kp,
...
private:
    ...
    std::vector<idx_pair> mRawIndexLst,
        mNewIndexLst;             // used for the topo info of system matrix.
    ...
    void CalcPoseDiffFromTorque(const tVectorXd &input_torque,
                                const tVectorXd &input_pose,
                                const tVectorXd &input_vel,
                                tVectorXd &output_vel_diff);
    void BuildTopo();
    void MatrixTransfer(Eigen::MatrixXd &raw_mat, Eigen::MatrixXd &new_mat,
                        int mode) const;
