                     ...
                     const Eigen::MatrixXd &body_defs, const tVector &gravity)
{
    ...
    tMatrix trans_mat; // 4*4的矩阵，成为tMatrix,
                       // 从名字上看trans_mat是一个"转换矩阵", transform matrix
...
cSpAlg::tSpTrans cRBDModel::GetSpWorldJointTrans(int j) const
{
    // 对于joint j，获取其世界转移
    assert(j >= 0 && j < GetNumJoints());
    cSpAlg::tSpTrans trans =
        cSpAlg::GetTrans(mSpWorldJointTransArr, j); // 一个(3, 4)的矩阵
    return trans;
}
...
cRBDModel::GetJointSubspace(int j) const
{
    assert(j >= 0 && j < GetNumJoints());
...
void cRBDModel::InitJointSubspaceArr()
{
    ...
    int num_joints = GetNumJoints();
    mJointSubspaceArr =
        Eigen::MatrixXd(cSpAlg::gSpVecSize, num_dofs); // 6 * dofs
    for (int j = 0; j < num_joints; ++j)
    {
        int offset = cKinTree::GetParamOffset(mJointMat, j);
        int dim = cKinTree::GetParamSize(mJointMat, j);
        int r = static_cast<int>(mJointSubspaceArr.rows());
        // given pose, calculates "subpsace" (a vector [gSpVEcSize, dof of
        // this joint]) for each joint just some rotation matrix in
        // mJointSubspaceArr
