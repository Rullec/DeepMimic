...
{
    std::cout << "bool cCharController::NeedNewAction() const = false"
              << std::endl;
    return false;
}
...
eActionSpace cCharController::GetActionSpace() const
{
    return eActionSpace::eActionSpaceContinuous;
}

int cCharController::GetStateSize() const { return 0; }

int cCharController::GetGoalSize() const { return 0; }

int cCharController::GetActionSize() const { return 0; }

int cCharController::GetNumActions() const { return 0; }

void cCharController::HandlePoseReset() {}

void cCharController::HandleVelReset() {}

void cCharController::BuildStateOffsetScale(Eigen::VectorXd &out_offset,
...
void cCharController::BuildGoalOffsetScale(Eigen::VectorXd &out_offset,
                                           Eigen::VectorXd &out_scale) const
{
...
void cCharController::BuildActionOffsetScale(Eigen::VectorXd &out_offset,
                                             Eigen::VectorXd &out_scale) const
{
...
void cCharController::BuildActionBounds(Eigen::VectorXd &out_min,
                                        Eigen::VectorXd &out_max) const
{
    int action_size = GetActionSize();
    out_min = -std::numeric_limits<double>::infinity() *
              Eigen::VectorXd::Ones(action_size);
...
int cCharController::NumChildren() const { return 0; }

const std::shared_ptr<cCharController> &cCharController::GetChild(int i) const
{
    return std::shared_ptr<cCharController>(nullptr);
}