...
cCtPDGenController::cCtPDGenController()
{
    ...
    mEnableCalcDeriv = false;
    mBufferDuDa.resize(0, 0);
    mBufferDuDx.resize(0, 0);
...
void cCtPDGenController::ClearBuffer()
{
    mBufferDuDx.resize(0, 0);
    mBufferDuDa.resize(0, 0);
}
                                       ...
                                       Eigen::VectorXd &out_tau)
{
    ...
    if (mEnableCalcDeriv == true)
    {
        mBufferDuDa = CalcDCtrlForceDAction(time_step);
        mBufferDuDx = CalcDCtrlForceDx(time_step);
        // std::cout << "duda size = " << mBufferDuDa.rows() << " "
        //           << mBufferDuDa.cols() << std::endl;
    }
...
/**
 * \brief           Get the derivative du/da
*/
tMatrixXd cCtPDGenController::GetDCtrlForceDAction() const
{
    // u is the control force, a is the action
    return mBufferDuDa;
}

tMatrixXd cCtPDGenController::GetDCtrlForceDxcur() const { return mBufferDuDx; }
...
/**
 * \brief        approximately calculate d(ctrl_force)/dx
 *      x = [q, qdot]
 *      ctrl_force is the generalized controler force generated by this SPD controller
 *      here, we calculate the partial derivative d(ctrl_force)/dx
 * 
 *      For more details, please check the note "20210112 计算控制力u对状态x的导数" 
 *      and another note "20210123 (最终版)状态函数f对x和u求梯度，系统思考.md"
*/
tMatrixXd cCtPDGenController::CalcDCtrlForceDx(double dt)
{
    return mPDGenController->CalcDCtrlForceDx_Approx(dt);
}

/**
 * \brief       test CalcDCtrlForceDx_Approx
*/
void cCtPDGenController::TestDCtrlForceDx_Approx()
{
    // 1. push current gen char pose
    auto gen_char = dynamic_cast<cSimCharacterGen *>(mChar);
    gen_char->PushState("test_dctrlforcedx_approx");
    // 2. get the gradient, record current ctrl force
    double dt = 1.6e-3;
    tMatrixXd ideal_dudx = CalcDCtrlForceDx(dt);
    tVectorXd old_q = gen_char->Getq(), old_qdot = gen_char->Getqdot();
    tVectorXd u_old;
    UpdatePDCtrls(dt, u_old);
    ...
    for (int i = 0; i < 2 * dof; i++)
    {
        if (i < dof)
        {
            // q
            old_q[i] += eps;
            gen_char->SetqAndqdot(old_q, old_qdot);
            old_q[i] -= eps;
            UpdatePDCtrls(dt, u_new);
            num_dudx = (u_new - u_old) / eps;
        }
        else
        {
            // qdot
        ...
        tVectorXd ideal_dudxi = ideal_dudx.col(i);
        tVectorXd diff_dudxi = num_dudx - ideal_dudxi;
        printf("[debug] dudx%d error %.3f%%\n", i,
               diff_dudxi.norm() / (num_dudx.norm() + 1e-16) * 100);
        std::cout << "[debug] approx = " << ideal_dudxi.transpose()
                  << std::endl;
        std::cout << "[debug] accurate = " << num_dudx.transpose() << std::endl;
        std::cout << "[debug] diff = " << diff_dudxi.transpose() << std::endl;
    }
    std::cout << "[debug] q = " << gen_char->Getq().transpose() << std::endl;
    std::cout << "[debug] qdot = " << gen_char->Getqdot().transpose()
              << std::endl;
    tVectorXd tq, tqdot;
    mPDGenController->GetPDTarget_q(tq, tqdot);
    std::cout << "[debug] target q = " << tq.transpose() << std::endl;
    // X. pop current gen char pose
    gen_char->PopState("test_dctrlforcedx_approx");
    // MIMIC_ERROR("hasn't been implemented\n");
}

/**
 * \brief           set enable to calculate the derivatives
 *                   of control forces to x or a
*/
void cCtPDGenController::SetEnableCalcDeriv(bool v) { mEnableCalcDeriv = v; }

/**
 * \brief           get whether do we need to calculate the derivatives
*/
bool cCtPDGenController::GetEnableCalcDeriv() const { return mEnableCalcDeriv; }