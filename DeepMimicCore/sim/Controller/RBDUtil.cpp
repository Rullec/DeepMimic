                            ...
                            Eigen::VectorXd &out_tau)
{
    ...
    for (int j = 0; j < num_joints; ++j)
    {
        if (cKinTree::IsValidBody(body_defs, j))
        {
            cSpAlg::tSpTrans parent_child_trans =
                model.GetSpParentChildTrans(j);
            cSpAlg::tSpTrans world_child_trans = model.GetSpWorldJointTrans(j);

            const auto S =
                model.GetJointSubspace(j); // ��ȡjoint�ӿռ�?joint�����ӿռ�?
            Eigen::VectorXd q;
            Eigen::VectorXd dq;
            Eigen::VectorXd ddq;
            // ��Joint j ��ȡ��ǰʱ�� pose, vel, accel
            ...
            cSpAlg::tSpVec curr_acc =
                cSpAlg::ApplyTransM(parent_child_trans, acc_p) + Sddq + cj +
                cSpAlg::CrossM(curr_vel, vj);
    ...
    for (int j = num_joints - 1; j >= 0; --j)
    {
        if (cKinTree::IsValidBody(body_defs, j))
        {
            ...
            {
                int parent_id = cKinTree::GetParent(joint_mat, j);
                cSpAlg::tSpTrans child_parent_trans =
                    model.GetSpChildParentTrans(j);
                fs.row(parent_id) +=
                    cSpAlg::ApplyTransF(child_parent_trans, curr_f);
            }
        }
    }
}
                            ...
                            Eigen::MatrixXd &out_mass_mat)
{
    ...
    Eigen::MatrixXd child_parent_mats_F =
        Eigen::MatrixXd(svs * num_joints, svs); // 6 * joints, 6
    Eigen::MatrixXd parent_child_mats_M =
        Eigen::MatrixXd(svs * num_joints, svs); // 6 * joints, 6
    Eigen::MatrixXd &Is =
        inertia_buffer; // inertia buffer? for each body? in which frame?
    for (int j = 0; j < num_joints; ++j)
    {
        if (cKinTree::IsValidBody(body_defs, j))
        {
            // Is: [6 * joints, 6]
            // this block: the spatial inertia of this body = combine
            // moment of inertia and mass
            Is.block(j * svs, 0, svs, svs) =
                BuildInertiaSpatialMat(body_defs, j);
        }
        ...
        if (cKinTree::IsValidBody(body_defs, j))
        {
            ...
            if (dim > 0)
            {
                ...
                while (cKinTree::HasParent(joint_mat, curr_id))
                {
                    ...
                    int curr_offset =
                        cKinTree::GetParamOffset(joint_mat, curr_id);
                                        ...
                                        Eigen::MatrixXd &out_J)
{
    ...
    while (curr_id != cKinTree::gInvalidJointID)
    {
        ...
        cSpAlg::tSpTrans parent_child_trans =
            model.GetSpParentChildTrans(curr_id);
...
void cRBDUtil::BuildJacobian(const cRBDModel &model, Eigen::MatrixXd &out_J)
{
    ...
    out_J = Eigen::MatrixXd::Zero(cSpAlg::gSpVecSize,
                                  num_dofs); // 6 * n��ȷʵ��[Jv, Jw]����ʽ
...
Eigen::MatrixXd
cRBDUtil::ExtractEndEffJacobian(const Eigen::MatrixXd &joint_mat,
                                const Eigen::MatrixXd &J, int joint_id)
{
    int curr_id = joint_id;
        ...
        if (cKinTree::IsValidBody(body_defs, j))
        {
            ...
            while (curr_id != cKinTree::gInvalidJointID)
            {
                ...
                auto J_com_block =
                    out_J.block(0, offset, cSpAlg::gSpVecSize, size);
                J_com_block +=
                    mass_frac * cSpAlg::ApplyTransM(body_pos_trans, J_block);
                                        ...
                                        const Eigen::MatrixXd &Jd, int joint_id)
{
    ...
    while (curr_id != cKinTree::gInvalidJointID)
    {
        ...
        if (cj.squaredNorm() > 0)
        {
            cSpAlg::tSpTrans world_joint_trans =
                model.GetSpWorldJointTrans(curr_id);
                       ...
                       tVector &out_com, tVector &out_vel)
{
    ...
    for (int j = 0; j < num_joints; ++j)
    {
        if (cKinTree::IsValidBody(body_defs, j))
        {
            tVector local_com = cKinTree::GetBodyLocalCoM(body_defs, j);
            tMatrix joint_world_mat =
                cKinTree::JointWorldTrans(joint_mat, pose, j);
            tMatrix body_joint_mat = cKinTree::BodyJointTrans(body_defs, j);
            tMatrix body_world_mat = joint_world_mat * body_joint_mat;
...
cSpAlg::tSpMat
cRBDUtil::BuildMomentInertiaCapsule(const Eigen::MatrixXd &body_defs,
                                    int part_id)
{
    const cKinTree::tBodyDef &def = body_defs.row(part_id);
    double mass = cKinTree::GetBodyMass(body_defs, part_id);
...
cSpAlg::tSpMat
cRBDUtil::BuildMomentInertiaSphere(const Eigen::MatrixXd &body_defs,
                                   int part_id)
{
    const cKinTree::tBodyDef &def = body_defs.row(part_id);
    double mass = cKinTree::GetBodyMass(body_defs, part_id);
...
cSpAlg::tSpMat
cRBDUtil::BuildMomentInertiaCylinder(const Eigen::MatrixXd &body_defs,
                                     int part_id)
{
    const cKinTree::tBodyDef &def = body_defs.row(part_id);
    double mass = cKinTree::GetBodyMass(body_defs, part_id);
...
cSpAlg::tSpMat
cRBDUtil::BuildInertiaSpatialMat(const Eigen::MatrixXd &body_defs, int part_id)
{
    ...
    */
    ...
    tMatrix body_joint =
        cKinTree::BodyJointTrans(body_defs, part_id); // body -> joint frame
    cSpAlg::tSpTrans X =
        cSpAlg::MatToTrans(body_joint); // rotation matrix to spatial trans
                                        ...
                                        Eigen::MatrixXd &out_trans_arr)
{
    ...
    int num_joints = cKinTree::GetNumJoints(joint_mat);
    out_trans_arr.resize(num_joints * cSpAlg::gSVTransRows,
                         cSpAlg::gSVTransCols);
...
cSpAlg::tSpTrans
cRBDUtil::BuildParentChildTransform(const Eigen::MatrixXd &joint_mat,
...
cSpAlg::tSpTrans
cRBDUtil::BuildChildParentTransform(const Eigen::MatrixXd &joint_mat,
                                    const Eigen::VectorXd &pose, int j)
{
...
Eigen::MatrixXd
cRBDUtil::BuildJointSubspaceRoot(const Eigen::MatrixXd &joint_mat,
                                 const Eigen::VectorXd &pose)
{
    ...
    Eigen::MatrixXd S =
        Eigen::MatrixXd::Zero(cSpAlg::gSpVecSize, dim); // S = [6, 7]
    tQuaternion quat = cKinTree::GetRootRot(joint_mat, pose);
    tMatrix E =
        cMathUtil::RotateMat(quat); // rotation matrix of root joint,  4*4

    S.block(3, 0, 3, pos_dim) =
        E.block(0, 0, 3, pos_dim).transpose(); // S[3,3] down left = rot_mat.T
...
Eigen::MatrixXd
cRBDUtil::BuildJointSubspaceRevolute(const Eigen::MatrixXd &joint_mat,
                                     const Eigen::VectorXd &pose, int j)
{
...
Eigen::MatrixXd
cRBDUtil::BuildJointSubspacePrismatic(const Eigen::MatrixXd &joint_mat,
                                      const Eigen::VectorXd &pose, int j)
{
...
Eigen::MatrixXd
cRBDUtil::BuildJointSubspacePlanar(const Eigen::MatrixXd &joint_mat,
                                   const Eigen::VectorXd &pose, int j)
{
...
Eigen::MatrixXd
cRBDUtil::BuildJointSubspaceFixed(const Eigen::MatrixXd &joint_mat,
                                  const Eigen::VectorXd &pose, int j)
{
...
Eigen::MatrixXd
cRBDUtil::BuildJointSubspaceSpherical(const Eigen::MatrixXd &joint_mat,
                                      const Eigen::VectorXd &pose, int j)
{
                                ...
                                Eigen::VectorXd &out_g_force)
{
    ...
    for (int j = 0; j < num_joints; ++j)
    {
        if (cKinTree::IsValidBody(body_defs, j))
        {
            ...
            cSpAlg::tSpMat I = BuildInertiaSpatialMat(body_defs, j);
            cSpAlg::tSpVec curr_acc =
                cSpAlg::ApplyTransM(world_child_trans, acc0);
