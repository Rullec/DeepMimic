...
new_data_summary_table_path = (
    "data/id_test/solved_leftleg/summary_leftleg.json"
)

split_intervals_num = 600
...
def get_state_action_staticstics_by_phase(table_path: str):
    ...
    with open("action.rec", 'w') as f_a, open("state.rec", 'w') as f_s:
        for i in range(split_intervals_num):
            state_norm_this_phase = np.linalg.norm(
                np.array(state_lst[i]), axis=1)
            state_mean_lst[i] = np.mean(state_norm_this_phase)
            state_std_lst[i] = np.std(state_norm_this_phase)

            action_norm_this_phase = np.linalg.norm(
                np.array(action_lst[i]), axis=1)
            ...
            print(
                f"split interval {i} action = \n{np.array(action_lst[i])}", file=f_a)
            print(
                f"split interval {i} state = \n{np.array(state_lst[i])}", file=f_s)
...
if __name__ == "__main__":
    ...
    new_s_mean, new_s_std, new_a_mean, new_a_std = get_state_action_staticstics_by_phase(
        os.path.join(cwd, new_data_summary_table_path))
    ...
    subplot_wrapper(new_s_mean, new_s_mean, "state mean", 1)
    subplot_wrapper(new_s_std, new_s_std, "state std", 2)
    subplot_wrapper(new_a_mean, new_a_mean, "action mean", 3)
    subplot_wrapper(new_a_std, new_a_std, "aciton std", 4)
    plt.show()
