...
weight_shape_info = [(275, 1024), (1024,), (1024, 512),
                     (512,), (512, 58), (58,)]
...
def select_key_pts(i, conf_lst,  k=5):
    ...
    '''
    conf_mat = np.array(
        conf_lst)   # 要求conf_mat.shape = (links_num, skeleton_num)
    # print("conf_mat shape = %s" % str(conf_mat.shape))

    # 将conf_mat[i]，也就是第i个conf重复70次，也形成一个70*18的矩阵
    target_vec = np.reshape(
        np.tile(conf_mat[i], conf_mat.shape[0]), conf_mat.shape)

    # 两个矩阵相减，并且对axis=1求l2-norm，得到一个70*1个列表，这就是各个conf到第i个conf的距离。
    norm2_res = np.linalg.norm(conf_mat - target_vec, axis=1)
    # 给上面那个70*1的列表，附加上一个0-69的id信息，形成一个矩阵为2*70, 其中第一行为0-69的顺序id, 第二行为对应的距离
    norm2_res_id = np.vstack([np.arange(0, len(conf_lst), 1), norm2_res])
...
def sample(theta_mat, conf_lst, K, batch_size=8):
    ...
    for _ in range(batch_size):
        ...
        # 把当前data合并到total里面
        def merge(total, item): return item if total is None else np.vstack(
            [total, item])
...
def single_theta_vec2dict(theta_vec, weight_name_lst, weight_shape_lst):
    ...
    def get_num(tup): return tup[0] if len(tup) == 1 else tup[0] * tup[1]

    # 获取shape中各个tuple所代表的参数个数
    theta_num_lst = [get_num(i) if len(
        i) <= 2 else AssertionError for i in weight_shape_lst]
    ...
    for i in range(len(weight_name_lst)):
        ...
        theta_dict[name] = np.reshape(
            theta_vec[cur_ptr: cur_ptr+theta_num_lst[i]], weight_shape_lst[i])
...
class autoencoder:
    ...
    def para_load(self, hpara_name):
        ...
        with open(hpara_path, "rb") as f:
            ...
            print("load hyper parameters from %s succ: %s" %
                  (hpara_path, str(load_para_dict)))
    ...
    def _build_encoder(self, inputs):
        ...
        pooling_input2 = tf.layers.max_pooling1d(
            inputs=pooling_input, pool_size=5, strides=3, padding="valid", name="pooling_1_enc")
        pooling_input2 = tf.layers.max_pooling1d(
            inputs=pooling_input2, pool_size=5, strides=3, padding="valid", name="pooling_2_enc")
        l1 = tf.layers.dense(inputs=pooling_input2, units=512,
                             activation=tf.nn.relu, reuse=tf.AUTO_REUSE,  name="l1_enc")
        l2 = tf.layers.dense(
            inputs=l1, units=256, activation=tf.nn.relu, reuse=tf.AUTO_REUSE, name="l2_enc")
        l3 = tf.layers.dense(
            inputs=l2, units=128, activation=tf.nn.relu, reuse=tf.AUTO_REUSE,  name="l3_enc")
        encoder_output = tf.layers.dense(
            inputs=l3, units=32, activation=tf.nn.relu, reuse=tf.AUTO_REUSE, name="output_encoder")
        return encoder_output

    def _build_decoder(self, inputs):
        l1 = tf.layers.dense(
            inputs=inputs, units=32, activation=tf.nn.relu, reuse=tf.AUTO_REUSE, name="l1_dec")
        l2 = tf.layers.dense(
            inputs=l1, units=128, activation=tf.nn.relu, reuse=tf.AUTO_REUSE, name="l2_dec")
        l3 = tf.layers.dense(
            inputs=l2, units=256, activation=tf.nn.relu, reuse=tf.AUTO_REUSE, name="l3_dec")
        l4 = tf.layers.dense(
            inputs=l3, units=512, activation=tf.nn.relu, reuse=tf.AUTO_REUSE, name="l4_dec")
        decoder_output = tf.layers.dense(
            inputs=l4, units=self.theta_num, activation=None, reuse=tf.AUTO_REUSE, name="output_decoder")
        return decoder_output

    def build_network(self):
        # inputs: K theta.shape = (None, K, theta_num), 2个动态维度
        self.input_theta_ph = tf.placeholder(
            tf.float32, shape=[None, None, self.theta_num], name="theta_ph")
        # inputs: weight.shape = (None, 1, K) 2个动态维度
        self.weight_ph = tf.placeholder(
            tf.float32, shape=[None, 1, None], name="weight_ph")
        # inputs: target_theta = (None, 1, theta_num) 1 个动态维度
        self.theta_target_ph = tf.placeholder(
            tf.float32, shape=[None, 1, self.theta_num], name="theta_target_ph")

        # encoder: shared variables
        with tf.variable_scope("encoder"):
            self.encoder_output = self._build_encoder(self.input_theta_ph)
            self.encoder_output_target = self._build_encoder(
                self.theta_target_ph)

        # decoder
        with tf.variable_scope("decoder"):
            self.decoder_output_utilize = self._build_decoder(
                tf.matmul(self.weight_ph, self.encoder_output))
            self.decoder_output = self._build_decoder(self.encoder_output)
            self.decoder_output_target = self._build_decoder(
                self.encoder_output_target)

        # define loss
        # loss1: 插值controller参数的恢复loss
        self.loss1 = self.loss1_weight * \
            tf.reduce_mean(tf.squared_difference(
                self.decoder_output, self.input_theta_ph))
        tf.reshape(self.loss1, (1, ))

        # loss2: target controller的恢复loss
        self.loss2 = self.loss2_weight * \
            tf.reduce_mean(tf.squared_difference(
                self.decoder_output_target, self.theta_target_ph))
        tf.reshape(self.loss2, (1, ))

        # loss3: 插值controller参数的code加权和 = target controller 的code
        self.loss3 = self.loss3_weight * tf.reduce_mean(tf.squared_difference(
            tf.matmul(self.weight_ph, self.encoder_output), self.encoder_output_target))
        ...
        clipped_grad = [(tf.clip_by_value(grad_, -1, 1), var)
        ...
        for i in tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES):
            self.net_weight_dict[i.name] = tf.get_default_graph(
            ).get_tensor_by_name(i.name)

        # session
        self.sess = tf.Session()
        self.log_saver = tf.summary.FileWriter(
            "logs/autoencoder_logs", self.sess.graph)
    ...
    def load(self, name, load_conf=True):
        ...
        for i in self.net_weight_dict:
            print("%s : %.2f" % (i, np.linalg.norm(
                self.sess.run(self.net_weight_dict[i]))))
    ...
    def train_one_step(self, theta_train, theta_target, weights):
        ...
        summary = tf.Summary()
        summary.value.add(tag="io_diff/mean",
                          simple_value=float(np.mean(diff)))
        summary.value.add(tag="io_diff/std", simple_value=float(np.std(diff)))
        summary.value.add(tag="Loss/loss1_插值controller的恢复",
                          simple_value=float(loss1))
        summary.value.add(tag="Loss/loss2_target_controller的恢复",
                          simple_value=float(loss2))
        summary.value.add(tag="Loss/loss3_code空间的线性组合关系",
                          simple_value=float(loss3))
        summary.value.add(tag="Loss_normalized/loss1(归一化)",
                          simple_value=float(loss1) / self.loss1_weight)
        summary.value.add(tag="Loss_normalized/loss2(归一化)",
                          simple_value=float(loss2) / self.loss2_weight)
        summary.value.add(tag="Loss_normalized/loss3(归一化)",
                          simple_value=float(loss3) / self.loss3_weight)
        ...
        print("\repoch %d, iter %d, loss:%.6f, lr = %.6f" %
              (self.epoch, self.iter, loss, self.lr), end='')
    ...
    def train_one_epoch(self, conf_lst, theta_mat, K, batch_size):
        ...
        for _ in range(0, data_num + batch_size, batch_size):
            # 获取数据
            train_data, target_data, weight_data, pt_id = sample(
                theta_mat, conf_lst, K, batch_size)
            # 进行训练
        ...
        if self.epoch % save_interval == 0:
            save_path = os.path.join(
                project_dir, "data0824/autoencoder_model/epoch%d-loss%.6f" % (self.epoch, avg_loss))
            self.save(save_path)
...
def train_mimic():
    ...
    parser.add_argument("--data_info_path", type=str, default=default_info_file_path,
                        help="read a info.json file for the skeleton-configuration pairs")
    arg = parser.parse_args()

    # 读取所有controller weight
    weight_paths, skeleton_paths = read_train_data_info(arg.data_info_path)
    theta_mat = read_batch_weight(
        weight_paths, weight_name_info, weight_shape_info)
...
def test_mimic():
    ...
    para_num = sum([i[0] * i[1] if 2 == len(i) else i[0]
                    for i in weight_shape_info])
    # sa = autoencoder(theta_num = para_num)

    # sample
    train_data_total, target_data_total, weight_data_total, center_pt_id = sample(
        weight_mat, skeleton_lst, 5, 8)
