            ...
            if frame_id % paint_sample == 0:
                ...
                if fetch_theta == True:
                    for sp_joint_id, aa_axis_st_pos in enumerate(st_num):
                        aa_theta_dist_with_respect_to_ref_time[sp_joint_id,
                                                               cur_phase_int] += cur_action[aa_axis_st_pos-1]
                elif fetch_axis == True:
                    for sp_joint_id, aa_axis_st_pos in enumerate(st_num):
                        aa_theta_dist_with_respect_to_ref_time[sp_joint_id,
                                                               cur_phase_int] += cur_action[aa_axis_st_pos]
...
if __name__ == "__main__":
    old_format_files = [os.path.join(old_format_data_dir, i)
                        for i in os.listdir(old_format_data_dir)]
    new_format_files = [[os.path.join(new_format_data_dir, i)] for i in os.listdir(
        new_format_data_dir) if i.find("train") != -1]
    ...
    pool = Pool(12)
    new_dist_mpi_lst = pool.map(
        get_new_data_action_theta_dist_with_ref_time_mpi, new_format_files)
