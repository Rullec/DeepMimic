...
def flipper_single_traj_file(origin_filename, target_filename):
    ...
    joint_axis_angle_theta_lst = action_axis_angle_theta_extract(
        origin_filename)
    # print(joint_axis_angle_theta_lst)
    joint_axis_angle_flipper_frame_lst = action_minimum_analyze(
...
def migrate_the_summary_table(origin_summary_table_path, target_train_data_dir):
    ...
    for id in range(len(trajs_lst)):
        ...
        target_train_data_file = os.path.join(
            target_train_data_dir, origin_train_data_file.split('/')[-1])
        trajs_lst[id]["train_data_filename"] = target_train_data_file
    summary_root["single_trajs_lst"] = trajs_lst

    target_summary_table_path = os.path.join(
        target_train_data_dir, origin_summary_table_path.split('/')[-1])
...
if __name__ == "__main__":
    ...
    files = [str(os.path.join(origin_train_data_dir, i))
             for i in os.listdir(origin_train_data_dir)]
    # files = files[ 0:10]
    # print(files)
    pool = Pool(12)
