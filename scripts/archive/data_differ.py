...
root_dir = ""
old_format_data = [str(i) for i in sorted(Path(
    "/home/xudong/Projects/DeepMimic/data/paths").iterdir(), key=os.path.getmtime)]
new_format_data = [str(i) for i in sorted(Path(
    "/home/xudong/Projects/DeepMimic/data/batch_train_data/0526").iterdir(), key=os.path.getmtime)]
...
if __name__ == "__main__":
    for data_id in range(len(new_format_data)):
        # 1. load buffers
        old_nums, old_states, old_actions, old_rewards = parse_old_format_data(
            os.path.join(root_dir, old_format_data[data_id]))
        new_nums, new_states, new_actions, new_rewards = parse_new_format_data(
            os.path.join(root_dir, new_format_data[data_id]), sample_num=20)
        logger.info(
            "---------------------data id %d--------------------", data_id)
        ...
        for i in range(old_nums):
            logger.info("for frame %d state: %lf - %lf", i, np.linalg.norm(
                np.array(old_states[i])), np.linalg.norm(np.array(new_states[i])))
            logger.info("for frame %d action: %lf - %lf", i, np.linalg.norm(
                np.array(old_actions[i])), np.linalg.norm(np.array(new_actions[i])))
            logger.info("for frame %d reward: %lf - %lf", i, np.linalg.norm(
                np.array(old_rewards[i])), np.linalg.norm(np.array(new_rewards[i])))
            ...
            if i != 0:
                state_diff += np.linalg.norm(
                    np.array(old_states[i]) - np.array(new_states[i]))
                action_diff += np.linalg.norm(
                    np.array(old_actions[i]) - np.array(new_actions[i]))
            logger.info("-----------------------------------")
