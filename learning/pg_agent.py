...
class PGAgent(TFAgent):
    ...
    def _build_nets(self, json_data):
        ...
        actor_init_output_scale = 1 if (
            self.ACTOR_INIT_OUTPUT_SCALE_KEY not in json_data) else json_data[self.ACTOR_INIT_OUTPUT_SCALE_KEY]
        ...
        self.g_tf = tf.placeholder(tf.float32, shape=(
            [None, g_size] if self.has_goal() else None), name="g")  # goals

        with tf.variable_scope('main'):
            with tf.variable_scope('actor'):
                self.actor_tf = self._build_net_actor(
    ...
    def _build_losses(self, json_data):
        ...
        if (critic_weight_decay != 0):
            self.critic_loss_tf += critic_weight_decay * \
        ...
        a_bound_loss = TFUtil.calc_bound_loss(
            norm_a_mean_tf, norm_a_bound_min, norm_a_bound_max)
        a_bound_loss /= self.exp_params_curr.noise
        self.actor_loss_tf += a_bound_loss

        if (actor_weight_decay != 0):
            self.actor_loss_tf += actor_weight_decay * \
                self._weight_decay_loss('main/actor')
    ...
    def _build_net_actor(self, net_name, init_output_scale):
        ...
        '''
        norm_s_tf = self.s_norm.normalize_tf(
            self.s_tf)  # 进入一个state的Normalizer里面, state正则化，并且在里面维护一个mean 和std
    ...
    def _update_actor(self):
        ...
        idx = self.replay_buffer.sample_filtered(
            self._local_mini_batch_size, key)
        ...
        loss, grads = self.sess.run(
            [self.actor_loss_tf, self.actor_grad_tf], feed)
    ...
    def _calc_action_logp(self, norm_action_deltas):
        ...
        a_size = self.get_action_size()
        logp = -0.5 / (stdev * stdev) * \
            np.sum(np.square(norm_action_deltas), axis=-1)
